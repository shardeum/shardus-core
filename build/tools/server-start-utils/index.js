...
class ServerStartUtils {
    constructor(config = {}) {
        ...
        if (!this.instanceNames)
            this.instanceNames = 'shardus-server';
        if (!this.serverPath)
            this.serverPath = path.join(this.baseDir, 'server.js');
        if (!this.Shardus)
            this.Shardus = require(path.join(this.baseDir, 'src', 'shardus'));
        if (!this.models)
            this.models = require(path.join(this.baseDir, 'src', 'storage', 'models'));
        if (!this.verbose)
            this.verbose = false;
        // Set NODE_UP_TIMEOUT if passed
        if (this.nodeUpTimeout)
            NODE_UP_TIMEOUT = this.nodeUpTimeout;
    ...
    async setServerConfig(port, changes = {}) {
        ...
        if (!server)
            return this._log('Could not find server on port', port);
        try {
    ...
    async stopServer(port) {
        ...
        if (server.status === 'stopped')
            return this._log('Server is already stopped on port', port);
    ...
    async deleteServer(port) {
        ...
        if (!server)
            return this._log('Could not find server on port', port);
        if (!['running', 'stopped'].includes(server.status))
            return;
        try {
            await this.stopServer(port);
        }
    ...
    async startServers(num, extPort = null, intPort = null, successFn = 'id', changes = null, outputToFile = true, instance = false, wait = 0) {
        if (!extPort)
            extPort = this.defaultConfigs.server.ip.externalPort;
        if (!intPort)
            intPort = this.defaultConfigs.server.ip.internalPort;
...
async function _forkServer(serverPath, extPort, baseDir, outputToFile = true, successFn) {
    ...
    if (!success)
        throw new Error(`Server at ${extPort} failed to start.`);
...
async function _startInstance(configs, successFn) {
    ...
    if (!success)
        throw new Error(`Server at ${configs.server.ip.externalPort} failed to start.`);
    return instance;
}
...
async function _awaitCondition(host, successFn, available = true) {
    ...
    while (true) {
        ...
        try {
            ...
            let successful;
            try {
                successful = successFn(resp.data);
            }
            catch (e) {
                throw e;
            }
