...
class P2PArchivers {
    ...
    registerRoutes() {
        this.p2p.network.registerExternalPost('joinarchiver', async (req, res) => {
            ...
            if (!accepted)
                return this.logDebug('Archiver join request not accepted.');
            this.logDebug('Archiver join request accepted!');
        });
        this.p2p.registerGossipHandler('joinarchiver', async (payload, sender, tracker) => {
            if (!this.state.acceptJoinRequests)
                return this.logDebug('Archiver join request not accepted. Not accepting join requests currently.');
...
//# sourceMappingURL=p2p-archivers.js.map