...
class P2PState extends EventEmitter {
    ...
    async _checkScaling() {
        ...
        switch (metadata.scaling) {
            case 'up':
                ...
                if (newDesired > this.maxNodes)
                    newDesired = this.maxNodes;
                break;
            case 'down':
                newDesired = metadata.startingDesired - this.amountToScale;
                // If newDesired less than minNodes, set newDesired to minNodes
                if (newDesired < this.minNodes)
                    newDesired = this.minNodes;
                break;
            default:
    ...
    async _addScalingRequest(scalingRequest) {
        ...
        if (!scalingRequest.node)
            return;
        // Check scaling seen for this node
        if (this.currentCycle.metadata.scalingSeen[scalingRequest.node])
            return;
    ...
    addLostMessage(msg, validate = false) {
        if (msg.lostMessage) { // Is DownMessage
            ...
            if (this.currentCycle.metadata.lostSeen.down[nodeId]) {
                ...
                if (updatesIdx > 0)
                    this.currentCycle.updates.lost.down.splice(updatesIdx, 1);
                const dataIdx = this.currentCycle.data.lost.findIndex(id => id === nodeId);
                if (dataIdx > 0)
                    this.currentCycle.updates.lost.down.splice(dataIdx, 1);
            }
    ...
    async _updateNodeStatus(node, status, updateDb = true) {
        ...
        if (oldStatus && this.nodes[oldStatus][id])
            delete this.nodes[oldStatus][id];
    ...
    _getOpenSlots() {
        ...
        if (needed === 0) {
            ...
            if (expired > 0)
                toAccept = expired;
            if (toAccept > this.maxNodesToRotate)
                toAccept = this.maxNodesToRotate;
        }
        else {
            ...
            if (toAccept > this.maxNodesPerCycle)
                toAccept = this.maxNodesPerCycle;
            ...
            if (toAccept > percentOfNodes)
                toAccept = percentOfNodes;
        }
        return toAccept;
    }
    ...
    _removeExcessNodes() {
        ...
        if (diff <= 0)
            return;
        let toRemove = diff;
        if (toRemove > this.maxNodesToRotate)
            toRemove = this.maxNodesToRotate;
        if (toRemove > expired)
            toRemove = expired;
    ...
    _addNodesToNodelist(nodes) {
        for (const node of nodes) {
            if (node.status)
                ...
                throw new Error('Node does not have status property');
        }
    }
    ...
    _isKnownNode(node) {
        const internalHost = `${node.internalIp}:${node.internalPort}`;
        if (!this.nodes.byIp[internalHost])
            return false;
        return true;
    }
    ...
    _createCycleMarker(gossip = true) {
        ...
        if (!this.cycles.length)
            return this.addCertificate({ marker: cycleMarker, signer: '0'.repeat(64) });
    ...
    getCycles(start = 0, end = this.cycles.length) {
        if (start < 0)
            throw new Error('Invalid start cycle counter.');
        if (end > this.cycles.length + 1)
            this.mainLogger.error('Invalid end cycle counter.');
    ...
    getLastCycleDuration() {
        const lastCycle = this.getLastCycle();
        if (!lastCycle)
            return this.defaultCycleDuration;
    ...
    getSeedNodes(forSeedList = true) {
        ...
        if (filteredNodes.length < this.minNodes - this.seedNodeOffset)
        ...
        if (filteredNodes.length <= this.maxSeedNodes)
            return produceSeedList(filteredNodes);
        // Remove excess nodes
...
//# sourceMappingURL=p2p-state.js.map