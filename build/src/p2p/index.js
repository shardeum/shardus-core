...
class P2P extends EventEmitter {
    ...
    async _getSeedNodes() {
        ...
        if (!this.crypto.verify(seedListSigned, archiver.publicKey))
        ...
        if (joinRequest) {
            if (this.archivers.addJoinRequest(joinRequest) === false)
                throw Error('Fatal: _getSeedNodes join request not accepted!');
        }
        if (dataRequest) {
            if (this.archivers.addDataRecipient(joinRequest.nodeInfo, dataRequest) === false)
                throw Error('Fatal: _getSeedNodes data request not accepted!');
        }
    ...
    async _ensureIpKnown() {
        ...
        if (!needsExternal && !needsInternal)
            return;
    ...
    _getNonSeedNodes(seedNodes = []) {
        ...
        if (!seedNodes.length)
            throw new Error('Fatal: No seed nodes provided!');
        ...
        class Tally {
            ...
            add(newItem, node) {
                ...
                for (let item of this.items) {
                    ...
                    if (!this.equalFn(newItem, item.value))
                        continue;
    ...
    async _sequentialQuery(nodes, queryFn, verifyFn) {
        ...
        for (let node of nodes) {
            try {
                ...
                if (!result)
                    throw new Error('Unable to get result from query.');
    ...
    async _syncUpChainAndNodelist() {
        ...
        if (lastCycleCounter === cycleCounter - 1)
            return true;
    ...
    async addJoinRequest(joinRequest, tracker, fromExternal = true) {
        ...
        if (!fromExternal)
            added = this.state.addGossipedJoinRequest(joinRequest);
        else {
            if (!active)
                return false;
    ...
    async _discoverNetwork(seedNodes) {
        ...
        try {
            ...
            if (!timeSynced)
                this.mainLogger.warn('Local time out of sync with time server.');
        }
        catch (e) {
    ...
    async _requestNetworkScaling(upOrDown) {
        if (!this.isActive() || this.scalingRequested)
            return;
    ...
    _wrapAndTagMessage(msg, tracker = '', recipientNode) {
        ...
        if (this.verboseLogs)
            this.mainLogger.debug(`Attaching sender ${this.id} to the message: ${utils.stringifyReduceLimit(msg)}`);
        const wrapped = {
            payload: msg,
            sender: this.id,
            tracker: tracker
    ...
    registerInternal(route, handler) {
        ...
        const wrappedHandler = async (wrappedPayload, respond) => {
            ...
            const respondWrapped = async (response) => {
                ...
                if (this.verboseLogs)
                    this.mainLogger.debug(`The signed wrapped response to send back: ${utils.stringifyReduceLimit(signedResponse)}`);
                if (route !== 'gossip') {
    ...
    async sendGossip(type, payload, tracker = '', sender = null, nodes = this.state.getAllNodes(this.id)) {
        ...
        if (this.verboseLogs)
            this.mainLogger.debug(`Start of sendGossip(${utils.stringifyReduce(payload)})`);
        ...
        try {
            if (this.verboseLogs)
                this.mainLogger.debug(`Gossiping ${type} request to these nodes: ${utils.stringifyReduce(recipients.map(node => utils.makeShortHash(node.id) + ':' + node.externalPort))}`);
            ...
            if (this.verboseLogs)
                this.mainLogger.error(`Failed to sendGossip(${utils.stringifyReduce(payload)}) Exception => ${ex}`);
            this.fatalLogger.fatal('sendGossip: ' + ex.name + ': ' + ex.message + ' at ' + ex.stack);
        }
        this.gossipedHashesSent.set(gossipHash, false);
        if (this.verboseLogs)
            this.mainLogger.debug(`End of sendGossip(${utils.stringifyReduce(payload)})`);
    }
    /**
    ...
    async sendGossipIn(type, payload, tracker = '', sender = null, nodes = this.state.getAllNodes()) {
        ...
        if (this.verboseLogs)
            this.mainLogger.debug(`Start of sendGossipIn(${utils.stringifyReduce(payload)})`);
        const gossipPayload = { type: type, data: payload };
        const gossipHash = this.crypto.hash(gossipPayload);
        if (this.gossipedHashesSent.has(gossipHash)) {
            if (this.verboseLogs)
        ...
        try {
            if (this.verboseLogs)
                this.mainLogger.debug(`GossipingIn ${type} request to these nodes: ${utils.stringifyReduce(recipients.map(node => utils.makeShortHash(node.id) + ':' + node.externalPort))}`);
            for (const node of recipients) {
        ...
        this.gossipedHashesSent.set(gossipHash, false);
        if (this.verboseLogs)
            this.mainLogger.debug(`End of sendGossipIn(${utils.stringifyReduce(payload)})`);
    }
    /**
   ...
   */
    async handleGossip(payload, sender, tracker = '') {
        if (this.verboseLogs)
            this.mainLogger.debug(`Start of handleGossip(${utils.stringifyReduce(payload)})`);
        ...
        if (this.verboseLogs)
            this.mainLogger.debug(`End of handleGossip(${utils.stringifyReduce(payload)})`);
    }
    /**
