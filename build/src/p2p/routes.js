...
function setupRoutes() {
    ...
    this.network.registerExternalPost('join', async (req, res) => {
        ...
        if (!accepted)
            return this.mainLogger.debug('Join request not accepted.');
    ...
    this.registerGossipHandler('join', async (payload, sender, tracker) => {
        if (!this.state.acceptJoinRequests || this.joinRequestToggle === false)
            return this.mainLogger.debug('Join request not accepted. Not accepting join requests currently.');
    ...
    this.registerGossipHandler('active', async (payload, sender, tracker) => {
        ...
        if (!added)
            return this.mainLogger.debug(`Status update to active for ${payload.nodeId} not added.`);
    ...
    this.registerGossipHandler('apoptosis', async (payload, sender, tracker) => {
        ...
        if (!added)
            return this.mainLogger.debug(`Apoptosis message for ${payload.nodeId} not added.`);
