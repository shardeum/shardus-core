...
class Shardus extends EventEmitter {
    ...
    async start(exitProcOnFail = true) {
        if (this.appProvided === null)
            throw new Error('Please call Shardus.setup with an App object or null before calling Shardus.start.');
    ...
    async syncAppData() {
        if (this.stateManager)
            await this.stateManager.syncStateData(3);
    ...
    put(tx, set = false) {
        ...
        try {
            ...
            if (this.verboseLogs)
                this.mainLogger.debug(`InitialValidationResponse: ${utils.stringifyReduce(initValidationResp)}`);
            ...
            if (this.verboseLogs)
                this.mainLogger.debug(`shardusTx. shortTxID: ${txId} txID: ${utils.makeShortHash(txId)} TX data: ${utils.stringifyReduce(shardusTx)}`);
            this.profiler.profileSectionStart('put');
            const signedShardusTx = this.crypto.sign(shardusTx);
            if (this.verboseLogs)
                this.mainLogger.debug('Transaction validated');
            ...
            this.consensus.inject(signedShardusTx)
                ...
                if (this.verboseLogs)
                    this.mainLogger.debug(`Received Consensus. Receipt: ${utils.stringifyReduce(txReceipt)}`);
            });
        }
        catch (err) {
