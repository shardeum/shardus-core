...
class Consensus extends EventEmitter {
    ...
    async inject(shardusTransaction) {
        ...
        try {
            ...
            if (this.mainLogs)
                this.mainLogger.debug(`Start of inject(${timestamp}  ${debugInfo}  tx: ${utils.stringifyReduce(shardusTransaction)})`);
            ...
            if (this.mainLogs)
                this.mainLogger.debug(`sourceAddress: ${utils.makeShortHash(sourceAddress)} targetAddress: ${utils.makeShortHash(targetAddress)}`);
            if (sourceAddress) {
                if (this.mainLogs)
                    this.mainLogger.debug(`get source state id for ${sourceAddress}`);
                stateId = null; // await this.app.getStateId(sourceAddress)
                if (this.mainLogs)
                    this.mainLogger.debug(`StateID: ${stateId} short stateID: ${utils.makeShortHash(stateId)} `);
            }
            if (targetAddress) {
                if (this.mainLogs)
            ...
            if (this.mainLogs)
                this.mainLogger.debug(`Creating the receipt for the transaction: StateID: ${stateId} short stateID: ${utils.makeShortHash(stateId)} `);
            transactionReceipt = this.createReceipt(inTransaction, stateId, targetStateId);
            if (this.mainLogs)
                this.mainLogger.debug(`Done Creating the receipt for the transaction: StateID: ${stateId} short stateID: ${utils.makeShortHash(stateId)} `);
        }
        catch (ex) {
        ...
        if (this.mainLogs)
            this.mainLogger.debug(`End of inject(${timestamp}  ${debugInfo})`);
