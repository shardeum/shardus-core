...
class StateManager extends EventEmitter {
    ...
    async syncStateTableData(lowAddress, highAddress, startTime, endTime) {
        ...
        while (searchingForGoodData) { // todo m11: this needs to be replaced
            ...
            let queryFn = async (node) => {
                ...
                if (result === false) {
                    this.mainLogger.error('ASK FAIL 1');
                }
                return result;
            };
            ...
            while (moreDataRemaining) {
                ...
                if (result === false) {
                    this.mainLogger.error('ASK FAIL 2');
                }
    ...
    async syncAccountData(lowAddress, highAddress) {
        ...
        while (moreDataRemaining) {
            ...
            if (result === false) {
                this.mainLogger.error('ASK FAIL 3');
            }
    ...
    async processAccountData() {
        ...
        if (failedHashes.length > 0) {
            ...
            for (let accountId of failedHashes) {
                ...
                if (this.verboseLogs)
                    this.mainLogger.debug(`DATASYNC: processAccountData ${accountId}  data: ${utils.stringifyReduce(account)}`);
                if (account != null) {
                    if (this.verboseLogs)
                        this.mainLogger.debug(`DATASYNC: processAccountData adding account to list`);
                    this.accountsWithStateConflict.push(account);
                }
                else {
                    if (this.verboseLogs)
                        this.mainLogger.debug(`DATASYNC: processAccountData cant find data: ${accountId}`);
                    if (accountId) {
    ...
    async writeCombinedAccountDataToBackups(failedHashes) {
        ...
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + 'writeCombinedAccountDataToBackups ' + accountCopies.length + ' ' + JSON.stringify(accountCopies[0]));
    ...
    async syncFailedAcccounts(lowAddress, highAddress) {
        ...
        if (this.verboseLogs)
            this.mainLogger.debug(`DATASYNC: syncFailedAcccounts start`);
        let addressList = [];
        for (let accountEntry of this.accountsWithStateConflict) {
            if (accountEntry.data && accountEntry.data.address) {
                ...
                if (this.verboseLogs)
                    this.mainLogger.debug(`DATASYNC: syncFailedAcccounts failed to add account ${accountEntry}`);
            }
        }
        ...
        if (result === false) {
            this.mainLogger.error('ASK FAIL 4');
        }
       ...
       */
        async (payload, respond) => {
            ...
            try {
                ...
                if (!payload) {
                    if (this.verboseLogs)
                        this.mainLogger.error(this.dataPhaseTag + ` _repair post_partition_results: abort no payload`);
                    return;
                }
                ...
                if (!payload.partitionResults) {
                    if (this.verboseLogs)
                        this.mainLogger.error(this.dataPhaseTag + ` _repair post_partition_results: abort, partitionResults == null`);
                    return;
                }
                if (payload.partitionResults.length === 0) {
                    if (this.verboseLogs)
                        this.mainLogger.error(this.dataPhaseTag + ` _repair post_partition_results: abort, partitionResults.length == 0`);
                    return;
                }
                if (this.verboseLogs && this.extendedRepairLogging)
                    this.mainLogger.debug(this.dataPhaseTag + ` _repair post_partition_results ${utils.stringifyReduce(payload)}`);
                if (!payload.partitionResults[0].sign) {
                    // TODO security need to check that this is signed by a valid and correct node
                    if (this.verboseLogs)
                        this.mainLogger.error(this.dataPhaseTag + ` _repair post_partition_results: abort, no sign object on partition`);
                    return;
                }
                ...
                for (let partitionResult of partitionResults) {
                    ...
                    if (partitionResult) {
                        ...
                        if (this.verboseLogs)
                            this.mainLogger.error(this.dataPhaseTag + ` _repair post_partition_results partitionResult missing`);
                    }
                    if (this.verboseLogs && this.extendedRepairLogging)
                        this.mainLogger.debug(this.dataPhaseTag + ` _repair post_partition_results partition: ${partitionResult.Partition_id} responses.length ${responses.length}  cycle:${payload.Cycle_number}`);
                }
                var partitionKeys = Object.keys(allResponsesByPartition);
                if (this.verboseLogs)
                    this.mainLogger.debug(this.dataPhaseTag + ` _repair post_partition_results partitionKeys: ${partitionKeys.length}`);
                ...
                for (let partitionKey of partitionKeys) {
                    ...
                    if (responses.length > 0) {
                        ...
                        if (repairTracker.busy && repairTracker.awaitWinningHash === false) {
                            if (this.verboseLogs)
                                this.mainLogger.debug(this.dataPhaseTag + ` _repair post_partition_results tracker busy. ${partitionKey} responses: ${responses.length}.  ${utils.stringifyReduce(repairTracker)}`);
                            continue;
                        }
                        if (repairTracker.repairsFullyComplete) {
                            if (this.verboseLogs)
                                this.mainLogger.debug(this.dataPhaseTag + ` _repair post_partition_results repairsFullyComplete = true  cycle:${payload.Cycle_number}`);
                            continue;
                        }
                    }
                    else {
                        if (this.verboseLogs)
                            this.mainLogger.debug(this.dataPhaseTag + ` _repair post_partition_results no responses. ${partitionKey} responses: ${responses.length}. repairTracker: ${utils.stringifyReduce(repairTracker)} responsesById: ${utils.stringifyReduce(allResponsesByPartition)}`);
                        continue;
                    }
                    ...
                    if (responses.length >= responsesRequired && (repairTracker.evaluationStarted === false || repairTracker.awaitWinningHash)) {
                        ...
                        if (ourResult == null) {
                            if (this.verboseLogs)
                                this.mainLogger.debug(this.dataPhaseTag + ` _repair post_partition_results our result is not computed yet `);
                        ...
                        if (!success) {
                            if (repairTracker.awaitWinningHash) {
                                if (topResult == null) {
                                    ...
                                    if (this.verboseLogs)
                                        this.mainLogger.debug(this.dataPhaseTag + ` _repair awaitWinningHash:true but topResult == null so keep waiting `);
                                    continue;
                                }
                                else {
                                    if (this.verboseLogs)
                                        this.mainLogger.debug(this.dataPhaseTag + ` _repair awaitWinningHash:true and we have a top result so start reparing! `);
                                }
                            }
                            if (this.resetAndApplyPerPartition === false && repairTracker.txRepairReady === true) {
                                if (this.verboseLogs)
                                    this.mainLogger.debug(this.dataPhaseTag + ` _repair txRepairReady:true bail here for some strange reason.. not sure aout this yet `);
                                continue;
                            }
                            if (this.verboseLogs)
                                this.mainLogger.debug(this.dataPhaseTag + ` _repair post_partition_results: tryGeneratePartitionReciept failed start repair process 1 ${utils.stringifyReduce(receiptResults)}`);
                            ...
                            if (this.verboseLogs)
                                this.mainLogger.debug(this.dataPhaseTag + ` _repair post_partition_results 3 allFinished, final cycle: ${payload.Cycle_number} hash:${utils.stringifyReduce({ topResult })}`);
                        ...
                        if (this.verboseLogs)
                            this.mainLogger.debug(this.dataPhaseTag + ` _repair post_partition_results not enough responses awaitWinningHash: ${repairTracker.awaitWinningHash} resp: ${responses.length}. required:${responsesRequired} repairTracker: ${utils.stringifyReduce(repairTracker)}`);
                    }
        ...
        this.p2p.registerInternal('get_transactions_by_partition_index', async (payload, respond) => {
            ...
            try {
                ...
                if (returnedResults < expectedResults) {
                    if (this.verboseLogs)
                        this.mainLogger.error(`get_transactions_by_partition_index failed! returnedResults < expectedResults send ${returnedResults} < ${expectedResults}`);
                }
                ...
                if (acceptedTXs != null && acceptedTXs.length < expectedResults) {
                    if (this.verboseLogs)
                        this.mainLogger.error(`get_transactions_by_partition_index results ${utils.stringifyReduce(acceptedTXs)} snippets ${utils.stringifyReduce(payload.debugSnippets)} `);
                    if (this.verboseLogs)
                        this.mainLogger.error(`get_transactions_by_partition_index results2:${utils.stringifyReduce(acceptedTXs.map(x => x.id))} snippets:${utils.stringifyReduce(payload.debugSnippets)} txid:${utils.stringifyReduce(txIDList)} `);
                    ...
                    if (this.verboseLogs)
                        this.mainLogger.error(`get_transactions_by_partition_index failed! returnedResults < expectedResults send3 ${acceptedTXs.length} < ${expectedResults} findsFixed: ${finds}  missing: ${utils.stringifyReduce(missingTXs)} found: ${utils.stringifyReduce(found)}`);
                }
                else {
                }
            }
    ...
    enableSyncCheck() {
        ...
        this._registerListener(this.p2p.state, 'newCycle', (cycles) => process.nextTick(async () => {
            ...
            let queryFn = async (node) => {
                let result = await this.p2p.ask(node, 'get_account_state_hash', message);
                if (result === false) {
                    this.mainLogger.error('ASK FAIL 5');
                }
                return result;
            };
    ...
    async restoreAccountDataByTx(nodes, accountStart, accountEnd, timeStart, timeEnd) {
        ...
        if (result === false) {
            this.mainLogger.error('ASK FAIL 6');
        }
    ...
    async restoreAccountData(nodes, accountStart, accountEnd, timeStart, timeEnd) {
        ...
        if (remoteAccountStates === false) {
            this.mainLogger.error('ASK FAIL 7');
        }
        ...
        if (accountData === false) {
            this.mainLogger.error('ASK FAIL 8');
        }
        if (accountData) {
        ...
        try {
            ...
            for (let accountEntry of accountData) {
                if (accountEntry.timestamp >= timestamp) {
                    failedAgeCheck = true;
                    if (this.verboseLogs)
                        this.mainLogger.debug(this.dataPhaseTag + 'testAccountTimesAndStateTable account has future state.  id: ' + utils.makeShortHash(accountEntry.accountId) + ' time: ' + accountEntry.timestamp + ' txTime: ' + timestamp + ' delta: ' + (timestamp - accountEntry.timestamp));
                }
            }
            if (failedAgeCheck) {
            ...
            if (Array.isArray(sourceKeys) && sourceKeys.length > 0) {
                ...
                if (accountStates.length !== 0) {
                    ...
                    if (accountStates.length === 0 || accountStates[0].stateBefore !== sourceState) {
                        if (this.verboseLogs)
                            console.log('testAccountTimesAndStateTable ' + timestamp + ' cant apply state 1');
                        if (this.verboseLogs)
                            this.mainLogger.debug(this.dataPhaseTag + 'testAccountTimesAndStateTable ' + timestamp + ' cant apply state 1 stateId: ' + utils.makeShortHash(sourceState) + ' stateTable: ' + utils.makeShortHash(accountStates[0].stateBefore) + ' address: ' + utils.makeShortHash(sourceAddress));
                        return { success: false, hasStateTableData };
                    }
                }
            }
            if (Array.isArray(targetKeys) && targetKeys.length > 0) {
                ...
                if (accountStates.length !== 0) {
                    ...
                    if (accountStates.length !== 0 && accountStates[0].stateBefore !== allZeroes64) {
                        ...
                        if (accountEntry == null) {
                            if (this.verboseLogs)
                                console.log('testAccountTimesAndStateTable ' + timestamp + ' target state does not exist. address: ' + utils.makeShortHash(targetAddress));
                            if (this.verboseLogs)
                                this.mainLogger.debug(this.dataPhaseTag + 'testAccountTimesAndStateTable ' + timestamp + ' target state does not exist. address: ' + utils.makeShortHash(targetAddress) + ' accountDataList: ' + utils.stringifyReduce(accountData));
                            ...
                            if (accountStates[0].stateBefore !== targetState) {
                                if (this.verboseLogs)
                                    console.log('testAccountTimesAndStateTable ' + timestamp + ' cant apply state 2');
                                if (this.verboseLogs)
                                    this.mainLogger.debug(this.dataPhaseTag + 'testAccountTimesAndStateTable ' + timestamp + ' cant apply state 2 stateId: ' + utils.makeShortHash(targetState) + ' stateTable: ' + utils.makeShortHash(accountStates[0].stateBefore) + ' address: ' + utils.makeShortHash(targetAddress));
                                return { success: false, hasStateTableData };
                            }
                        }
                    }
                        ...
                        if (accountStates.length !== 0 && accountStates[0].stateBefore !== allZeroes64) {
                            ...
                            if (accountEntry == null) {
                                if (this.verboseLogs)
                                    console.log('testAccountTimesAndStateTable ' + timestamp + ' target state does not exist. address: ' + utils.makeShortHash(targetAddress));
                                if (this.verboseLogs)
                                    this.mainLogger.debug(this.dataPhaseTag + 'testAccountTimesAndStateTable ' + timestamp + ' target state does not exist. address: ' + utils.makeShortHash(targetAddress) + ' accountDataList: ');
        ...
        try {
            ...
            for (let key of accountKeys) {
                ...
                if (accountEntry.timestamp >= timestamp) {
                    failedAgeCheck = true;
                    if (this.verboseLogs)
                        this.mainLogger.debug(this.dataPhaseTag + 'testAccountTime account has future state.  id: ' + utils.makeShortHash(accountEntry.accountId) + ' time: ' + accountEntry.timestamp + ' txTime: ' + timestamp + ' delta: ' + (timestamp - accountEntry.timestamp));
                }
            }
            if (failedAgeCheck) {
                // if (this.verboseLogs) this.mainLogger.debug('DATASYNC: testAccountTimesAndStateTable accounts have future state ' + timestamp)
    ...
    async tryApplyTransaction(acceptedTX, hasStateTableData, repairing, filter, wrappedStates, localCachedData) {
        ...
        try {
            ...
            txTs = timestamp;
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + `tryApplyTransaction  ts:${timestamp} repairing:${repairing}  Applying! ` + debugInfo);
            if (repairing !== true) {
                ...
                if (this.verboseLogs && this.extendedRepairLogging)
                    this.mainLogger.debug(this.dataPhaseTag + ` _repair tryApplyTransaction FIFO lock outer: ${utils.stringifyReduce(accountKeys)} `);
                ourAccountLocks = await this.bulkFifoLockAccounts(accountKeys);
                if (this.verboseLogs && this.extendedRepairLogging)
                    this.mainLogger.debug(this.dataPhaseTag + ` _repair tryApplyTransaction FIFO lock inner: ${utils.stringifyReduce(accountKeys)} `);
            }
            ourLockID = await this.fifoLock('accountModification');
            if (this.verboseLogs)
                console.log(`tryApplyTransaction  ts:${timestamp} repairing:${repairing}  Applying!`);
            ...
            if (hasStateTableData === false) {
                for (let stateT of stateTableResults) {
                    if (this.verboseLogs)
                        console.log('writeStateTable ' + utils.makeShortHash(stateT.accountId) + ' accounts total' + accountDataList.length);
                    if (this.verboseLogs)
                        this.mainLogger.debug(this.dataPhaseTag + 'writeStateTable ' + utils.makeShortHash(stateT.accountId) + ' before: ' + utils.makeShortHash(stateT.stateBefore) + ' after: ' + utils.makeShortHash(stateT.stateAfter) + ' txid: ' + utils.makeShortHash(acceptedTX.id) + ' ts: ' + acceptedTX.timestamp);
                }
            ...
            if (!repairing)
                this.tempRecordTXByCycle(txTs, acceptedTX, false, applyResponse);
            return false;
        }
        finally {
            this.fifoUnlock('accountModification', ourLockID);
            if (repairing !== true) {
                ...
                if (this.verboseLogs)
                    this.mainLogger.debug(this.dataPhaseTag + ` _repair tryApplyTransaction FIFO unlock inner: ${utils.stringifyReduce(accountKeys)} `);
            }
        }
    ...
    async applyAcceptedTransaction(acceptedTX, wrappedStates, localCachedData, filter) {
        ...
        if (this.verboseLogs)
            console.log('applyAcceptedTransaction ' + timestamp + ' ' + debugInfo);
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + 'applyAcceptedTransaction ' + timestamp + ' ' + debugInfo);
        let allkeys = [];
        ...
        for (let key of allkeys) {
            if (wrappedStates[key] == null) {
                if (this.verboseLogs)
                    console.log(`applyAcceptedTransaction missing some account data. timestamp:${timestamp}  key: ${utils.makeShortHash(key)}  debuginfo:${debugInfo}`);
        ...
        if (!success) {
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + 'applyAcceptedTransaction pretest failed: ' + timestamp);
        ...
        if (applyResult) {
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + 'applyAcceptedTransaction SUCCEDED ' + timestamp);
    ...
    updateHomeInformation(txQueueEntry) {
        if (this.currentCycleShardData != null && txQueueEntry.hasShardInfo === false) {
            ...
            for (let key of txQueueEntry.txKeys.allKeys) {
                ...
                if (homeNode == null) {
                    if (this.verboseLogs)
                        this.mainLogger.error(this.dataPhaseTag + ` queueAcceptedTransaction: ${key} `);
                }
    ...
    async queueEntryRequestMissingData(queueEntry) {
        ...
        for (let key of queueEntry.uniqueKeys) {
            if (queueEntry.collectedData[key] == null && queueEntry.requests[key] == null) {
                ...
                if (result === false) {
                    this.mainLogger.error('ASK FAIL 9');
                }
                        ...
                        try {
                            ...
                            if (txResult.success) {
                                ...
                                if (!edgeFailDetected && acceptedTXCount > 0) {
                                    ...
                                    if (this.verboseLogs)
                                        this.mainLogger.debug(this.dataPhaseTag + `processAcceptedTxQueue edgeFail ${utils.stringifyReduce(queueEntry.acceptedTx)}`);
    ...
    async getLocalOrRemoteAccount(address) {
        ...
        if (accountIsRemote) {
            ...
            if (result === false) {
                this.mainLogger.error('ASK FAIL 10');
            }
    ...
    async getRemoteAccount(address) {
        ...
        let result = await this.p2p.ask(homeNode.node, 'get_account_data_with_queue_hints', message);
        if (result === false) {
            this.mainLogger.error('ASK FAIL 11');
        }
        if (result != null && result.accountData != null && result.accountData.length > 0) {
            wrappedAccount = result.accountData[0];
            return wrappedAccount;
    ...
    generatePartitionResult(partitionObject) {
        ...
        if (this.verboseLogs && this.extendedRepairLogging)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair partitionObject: ${utils.stringifyReduce(partitionObject)}`);
        if (this.verboseLogs && this.extendedRepairLogging)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair generatePartitionResult: ${utils.stringifyReduce(partitionResult)}`);
    ...
    generatePartitionObject(lastCycle, partitionId) {
        ...
        let partitionObject = {
            ...
            Txids: txSourceData.hashes,
            Status: txSourceData.passed,
            States: txSourceData.states,
    ...
    tryGeneratePartitionReciept(allResults, ourResult, repairPassHack = false) {
        ...
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair  ${debugKey} tryGeneratePartitoinReciept repairTracker: ${utils.stringifyReduce(repairTracker)} other: ${utils.stringifyReduce({ topHash, topCount, topResult })}`);
        ...
        if (this.useHashSets && repairPassHack) {
            ...
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair  ${debugKey} tryGeneratePartitoinReciept hack force win: ${utils.stringifyReduce(repairTracker)} other: ${utils.stringifyReduce({ topHash, topCount, topResult })}`);
        }
        let resultsList = [];
        if (topCount >= requiredHalf) {
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair  ${debugKey} tryGeneratePartitoinReciept: top hash wins: ` + utils.makeShortHash(topHash) + ` ourResult: ${utils.makeShortHash(ourResult.Partition_hash)}  count/required ${topCount} / ${requiredHalf}`);
            ...
            if (this.useHashSets) {
                ...
                if (this.verboseLogs)
                    this.mainLogger.debug(this.dataPhaseTag + ` _repair  ${debugKey} tryGeneratePartitoinReciept: did not win, useHashSets: ` + utils.makeShortHash(topHash) + ` ourResult: ${utils.makeShortHash(ourResult.Partition_hash)}  count/required ${topCount} / ${requiredHalf}`);
                return { partitionReceipt: null, topResult: null, success: false };
            }
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair  ${debugKey} tryGeneratePartitoinReciept: top hash failed: ` + utils.makeShortHash(topHash) + ` ${topCount} / ${requiredHalf}`);
            return { partitionReceipt: null, topResult, success: false };
        }
        if (ourResult.Partition_hash !== topHash) {
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair  ${debugKey} tryGeneratePartitoinReciept: our hash does not match: ` + utils.makeShortHash(topHash) + ` our hash: ${ourResult.Partition_hash}`);
            return { partitionReceipt: null, topResult, success: false };
        }
        ...
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair  ${debugKey} tryGeneratePartitoinReciept OK! ${utils.stringifyReduce({ partitionReceipt, topResult })}`);
    ...
    async startRepairProcess(cycle, topResult, partitionId, ourLastResultHash) {
        ...
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair startRepairProcess begin.  repairTracker: ${utils.stringifyReduce(repairTracker)}`);
        ...
        try {
            ...
            if (this.repairStartedMap.has(combinedKey)) {
                if (this.verboseLogs)
                    this.mainLogger.error(`repairStats: Already started repair on ${combinedKey}`);
            }
            else {
                ...
                if (this.verboseLogs)
                    this.mainLogger.debug(`repairStats: staring repair ${combinedKey}`);
            }
            topResult = null; // hack
            if (topResult) {
                ...
                if (this.useHashSets) {
                    ...
                    if (retCode === 100) {
                        ...
                        if (this.verboseLogs)
                            this.mainLogger.error(this.dataPhaseTag + ` syncTXsFromHashSetStrings failed so we will set awaitWinningHash=true and hope a hash code wins ${debugKey} `);
                        return;
                    }
            ...
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair startRepairProcess syncTXs finished. ${debugKey} `);
            ...
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair startRepairProcess our result: ${debugKey} ${utils.stringifyReduce(ourResult)} obj: ${utils.stringifyReduce(this.partitionObjectsByCycle[key])} `);
            ...
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair startRepairProcess tryGeneratePartitionReciept: ${debugKey} ${utils.stringifyReduce({ partitionReceipt, topResult2, success })}  `);
            if (!success) {
                if (this.verboseLogs)
                    this.mainLogger.debug(this.dataPhaseTag + ` _repair startRepairProcess success==false starting repair again in 3 seconds! ${debugKey}`);
                ...
                if (this.verboseLogs)
                    this.mainLogger.debug(this.dataPhaseTag + ` _repair startRepairProcess success==ok ${debugKey}`);
                ...
                if (!repairsByPartition) {
                    if (this.verboseLogs)
                        this.mainLogger.debug(this.dataPhaseTag + ` _repair startRepairProcess repairsByPartition==null ${debugKey} ck: ${cycleKey} `);
                }
                ...
                if (this.resetAndApplyPerPartition) {
                    ...
                    for (let partitionKey of repairKeys) {
                        ...
                        if ((repairTracker1.txRepairComplete === false && repairTracker1.evaluationStarted) || repairTracker1.evaluationStarted === false) {
                            if (this.verboseLogs)
                                this.mainLogger.debug(this.dataPhaseTag + ` _repair startRepairProcess repairTracker1 ${debugKey} txRepairComplete === false ${utils.stringifyReduce(repairTracker1)} `);
                    ...
                    if (allFinished) {
                        if (this.verboseLogs)
                            this.mainLogger.debug(this.dataPhaseTag + ` _repair startRepairProcess allFinished, start merge and apply ${debugKey}`);
                        await this.mergeAndApplyTXRepairs(cycleNumber, partitionId);
                        // only declare victory after we matched hashes
                        if (usedSyncTXsFromHashSetStrings === false) {
                            if (this.verboseLogs)
                                this.mainLogger.debug(this.dataPhaseTag + ` _repair startRepairProcess 1 allFinished, final ${debugKey} hash:${utils.stringifyReduce({ topResult2 })}`);
                            this.repairTrackerMarkFinished(repairTracker, 'startRepairProcess:A');
                            return;
                        }
                        else {
                            if (this.verboseLogs)
                                this.mainLogger.debug(this.dataPhaseTag + ` _repair startRepairProcess set evaluationStarted=false so we can tally up hashes again ${debugKey}`);
                            ...
                            if (this.verboseLogs && this.extendedRepairLogging)
                                this.mainLogger.debug(this.dataPhaseTag + ` _repair startRepairProcess immediate receipt check. ${debugKey} success:${success3} topResult:${utils.stringifyReduce(topResult3)}  partitionReceipt: ${utils.stringifyReduce({ partitionReceipt3 })}`);
                            ...
                            if (!success3) {
                                if (repairTracker.awaitWinningHash) {
                                    if (topResult3 == null) {
                                        // if we are awaitWinningHash then wait for a top result before we start repair process again
                                        if (this.verboseLogs)
                                            this.mainLogger.debug(this.dataPhaseTag + ` _repair startRepairProcess awaitWinningHash:true but topResult == null so keep waiting ${debugKey}`);
                                    }
                                    else {
                                        if (this.verboseLogs)
                                            this.mainLogger.debug(this.dataPhaseTag + ` _repair startRepairProcess awaitWinningHash:true and we have a top result so start reparing! ${debugKey}`);
                                        if (this.verboseLogs)
                                            this.mainLogger.debug(this.dataPhaseTag + ` _repair startRepairProcess: tryGeneratePartitionReciept failed start repair process 2 ${debugKey}  ${utils.stringifyReduce(receiptResults)}`);
                                        let cycle = this.p2p.state.getCycleByCounter(cycleNumber);
                                        await utils.sleep(1000);
                                ...
                                if (this.verboseLogs)
                                    this.mainLogger.debug(this.dataPhaseTag + ` _repair startRepairProcess 2 allFinished, final ${debugKey} hash:${utils.stringifyReduce({ topResult3 })}`);
                            }
                        }
                    }
                }
            ...
            if (this.verboseLogs && this.extendedRepairLogging)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair startRepairProcess we repaired stuff so re-broadcast our results ${debugKey}`);
    ...
    async checkForGoodPartitionReciept(cycleNumber, partitionId) {
        ...
        let { partitionReceipt: partitionReceipt3, topResult: topResult3, success: success3 } = receiptResults;
        if (this.verboseLogs && this.extendedRepairLogging)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair checkForGoodPartitionReciept immediate receipt check. ${debugKey} success:${success3} topResult:${utils.stringifyReduce(topResult3)}  partitionReceipt: ${utils.stringifyReduce({ partitionReceipt3 })}`);
        // see if we already have a winning hash to correct to
        if (!success3) {
            if (repairTracker.awaitWinningHash) {
                if (topResult3 == null) {
                    // if we are awaitWinningHash then wait for a top result before we start repair process again
                    if (this.verboseLogs)
                        this.mainLogger.debug(this.dataPhaseTag + ` _repair checkForGoodPartitionReciept awaitWinningHash:true but topResult == null so keep waiting ${debugKey}`);
                }
                else {
                    if (this.verboseLogs)
                        this.mainLogger.debug(this.dataPhaseTag + ` _repair checkForGoodPartitionReciept awaitWinningHash:true and we have a top result so start reparing! ${debugKey}`);
                    if (this.verboseLogs)
                        this.mainLogger.debug(this.dataPhaseTag + ` _repair checkForGoodPartitionReciept: tryGeneratePartitionReciept failed start repair process 3 ${debugKey} ${utils.stringifyReduce(receiptResults)}`);
                    let cycle = this.p2p.state.getCycleByCounter(cycleNumber);
                    await utils.sleep(1000);
                    await this.startRepairProcess(cycle, topResult3, partitionId, ourResult.Partition_hash);
            ...
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair checkForGoodPartitionReciept 2 allFinished, final ${debugKey} hash:${utils.stringifyReduce({ topResult3 })}`);
        }
    }
    /**
    ...
    async syncTXsFromWinningHash(topResult) {
        ...
        if (!allNodes) {
            if (this.verboseLogs)
                this.mainLogger.error(this.dataPhaseTag + ' _repair syncTXsFromWinningHash: allNodes: undefined ');
        ...
        if (nodeToContact) {
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + ' _repair syncTXsFromWinningHash: node to ask for TXs: ' + utils.stringifyReduce(topResult));
        }
        else {
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + ' _repair syncTXsFromWinningHash: cant find node: ' + utils.stringifyReduce(topResult));
        ...
        if (partitionObject === false) {
            this.mainLogger.error('ASK FAIL 12');
        }
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + ' _repair syncTXsFromWinningHash: partitionObject: ' + utils.stringifyReduce(partitionObject));
        ...
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + ' _repair syncTXsFromWinningHash: ourPartitionObj: ' + utils.stringifyReduce(ourPartitionObj));
        ...
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + ' _repair syncTXsFromWinningHash: _getRepairTrackerForCycle: ' + utils.stringifyReduce(repairTracker));
        ...
        if (txs === false) {
            this.mainLogger.error('ASK FAIL 13');
        }
        ...
        if (txs === false) {
            this.mainLogger.error('ASK FAIL 14');
        }
        ...
        if (this.verboseLogs && this.extendedRepairLogging)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair syncTXsFromWinningHash: extraFailedState:${extraFailedState} extraFailedStatus:${extraFailedStatus} repairTracker updates: ` + utils.stringifyReduce(repairTracker) + ` invalidAcceptedTxIDs: ${invalidAcceptedTxIDs.length}`);
    ...
    _mergeRepairDataIntoLocalState(repairTracker, ourPartitionObj, otherStatusMap, otherPartitionObject) {
        ...
        newTxList.txs = txs;
        if (this.verboseLogs && this.extendedRepairLogging)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair _mergeRepairDataIntoLocalState:  key: ${key} txlist: ${utils.stringifyReduce({ hashes: txList.hashes, passed: txList.passed })} `);
    }
    ...
    _mergeRepairDataIntoLocalState2(repairTracker, ourPartitionObj, ourLastResultHash, ourHashSet, txListOverride = null) {
        ...
        let debugKey = `rkeys: ${key} ${key2}`;
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + `_mergeRepairDataIntoLocalState2 ${debugKey}`);
        ...
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + `_mergeRepairDataIntoLocalState2 z  ${debugKey} ourHashSet.extraMap: ${utils.stringifyReduce(ourHashSet.extraMap)} debugSol: ${utils.stringifyReduce(debugSol)}`);
        ...
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + `_mergeRepairDataIntoLocalState2 a  ${debugKey} len: ${ourHashSet.indexMap.length}  extraIndex: ${extraIndex} ourPreHashSet: ${hashSet}`);
        ...
        try {
            for (let i = 0; i < ourHashSet.indexMap.length; i++) {
                ...
                if (currentIndex >= 0) {
                    ...
                    if (newTxList.hashes[i] == null) {
                        if (this.verboseLogs)
                            this.mainLogger.error(this.dataPhaseTag + `_mergeRepairDataIntoLocalState2 ${debugKey} a error null at i: ${i}  solutionIndex: ${solutionIndex}  ourCounter: ${ourCounter}  newTxList.hashes.length: ${newTxList.hashes.length} newTxList.thashes.length: ${newTxList.thashes.length} ourHashSet.indexMap.length:${ourHashSet.indexMap.length} txSourceList.hashes.length: ${txSourceList.hashes.length}`);
                        if (this.verboseLogs)
                            this.mainLogger.error(this.dataPhaseTag + `_mergeRepairDataIntoLocalState2 ${debugKey} a error null at i: ${i}  newTxList:${utils.stringifyReduce(newTxList)}`);
                        if (this.verboseLogs)
                            this.mainLogger.error(this.dataPhaseTag + `_mergeRepairDataIntoLocalState2 ${debugKey} a error null at i: ${i}  txSourceList:${utils.stringifyReduce(txSourceList)}`);
                    ...
                    if (!solutionDelta) {
                        if (this.verboseLogs)
                            this.mainLogger.error(this.dataPhaseTag + `_mergeRepairDataIntoLocalState2 a error solutionDelta=null  solutionIndex: ${solutionIndex} i:${i} of ${ourHashSet.indexMap.length} deltas: ${utils.stringifyReduce(repairTracker.solutionDeltas)}`);
                    }
                    // insert the next one
                    ...
                    if (newTxList.hashes[i] == null) {
                        if (this.verboseLogs)
                            this.mainLogger.error(this.dataPhaseTag + `_mergeRepairDataIntoLocalState2 b error null at i: ${i}  solutionIndex: ${solutionIndex}  ourCounter: ${ourCounter}`);
                    }
                }
            }
        }
            ...
            if (this.verboseLogs)
                this.mainLogger.error('failnugget2:' + stringify({ txList, repairTracker, ourHashSet }));
        ...
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + `_mergeRepairDataIntoLocalState2 c  len: ${ourHashSet.indexMap.length}  solutionIndex: ${solutionIndex}  ourCounter: ${ourCounter} ourHashSet: ${hashSet}`);
    ...
    async syncTXsFromHashSetStrings(cycleNumber, partitionId, repairTracker, ourLastResultHash) {
        ...
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + `syncTXsFromHashSetStrings: ${debugKey}`);
        ...
        for (let hashSetEntry of hashSetList) {
            ...
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + JSON.stringify(hashSetEntry));
        }
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + JSON.stringify(output));
        // find our solution
        let ourSolution = hashSetList.find((a) => a.ourRow === true); // owner
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + 'ourSolution: ' + JSON.stringify({ ourSolution, len: ourSolution.hash.length }));
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + 'solved outputHashSet: ' + outputHashSet);
            ...
            if (correction.t === 'insert') {
                ...
                for (let i = 0; i < voters.length; i++) {
                    if (voters[i] === hashSetList.length - 1) {
                        ...
                        if (this.verboseLogs)
                            this.mainLogger.debug(this.dataPhaseTag + `skipped greedy index (type1) for FORCED ${voters[i]}`);
                        continue;
                    }
                ...
                if (greedyAsk < 0) {
                    ...
                    do {
                        ...
                        findValidIndex++;
                        if (this.verboseLogs)
                            this.mainLogger.debug(this.dataPhaseTag + `skipped greedy index (type2) for FORCED ${greedyAsk}`);
        ...
        for (let i = 0; i < requestsByHost.length; i++) {
            if (requestsByHost[i] != null) {
                ...
                if (this.extendedRepairLogging)
                    console.log(`get_transactions_by_partition_index ok! ${debugKey} payload: ${utils.stringifyReduce(payload)}`);
                if (this.extendedRepairLogging)
                    console.log(`requestsByHost[i].stateSnippets ${debugKey} ${utils.stringifyReduce(requestsByHost[i].stateSnippets)} `);
                if (hashSetList[i].owners.length > 0) {
                    ...
                    if (result === false) {
                        this.mainLogger.error('ASK FAIL 15');
                    }
                    ...
                    if (result.success === true) {
                        ...
                        if (this.extendedRepairLogging)
                            console.log(`get_transactions_by_partition_index ok! ${debugKey} count:${result.acceptedTX.length} payload: ${utils.stringifyReduce(payload)} reqCount:${payload.tx_indicies.length}`);
                        for (let j = 0; j < result.acceptedTX.length; j++) {
                            ...
                            if (acceptedTX == null) {
                                if (this.verboseLogs)
                                    this.mainLogger.error(`syncTXsFromHashSetStrings acceptedTX == null ${debugKey} j:${j} i:${i} pf:${result.passFail[j]}`);
                            }
                        ...
                        if (returnedResults < expectedResults) {
                            if (this.verboseLogs)
                                this.mainLogger.error(`get_transactions_by_partition_index failed! returnedResults < expectedResults recv ${returnedResults} < ${expectedResults}`);
                        }
                    }
                    else {
                        // todo datasync:  assert/fail/or retry
                        if (this.verboseLogs)
                            this.mainLogger.error(`get_transactions_by_partition_index failed! ${debugKey} payload: ${utils.stringifyReduce(payload)}`);
                    }
        ...
        if (insertCount !== repairTracker.solutionDeltas.length) {
            if (this.verboseLogs)
                this.mainLogger.error(`${debugKey} insertCount !== repairTracker.solutionDeltas.length: ${insertCount} , ${repairTracker.solutionDeltas.length} `);
        }
    ...
    _getRepairTrackerForCycle(counter, partition) {
        ...
        if (!repairTracker) {
            ...
            if (this.verboseLogs && this.extendedRepairLogging)
                this.mainLogger.debug(this.dataPhaseTag + `_getRepairTrackerForCycle: creating for cycle:${counter} partition:${partition}`);
            repairTracker = { triedHashes: [],
                numNodes: this.lastActiveNodeCount,
                counter: counter,
    ...
    repairTrackerMarkFinished(repairTracker, debugTag) {
        ...
        if (this.repairStartedMap.has(combinedKey)) {
            if (this.repairCompletedMap.has(combinedKey)) {
                if (this.verboseLogs)
                    this.mainLogger.debug(`repairStats: finished repair ${combinedKey} -alreadyFlagged  tag:${debugTag}`);
            }
            else {
                this.dataRepairsCompleted++;
                this.repairCompletedMap.set(combinedKey, true);
                if (this.verboseLogs)
                    this.mainLogger.debug(`repairStats: finished repair ${combinedKey} tag:${debugTag}`);
            }
        }
        else {
            // should be a trace?
            if (this.verboseLogs)
                this.mainLogger.debug(`repairStats: Calling complete on a key we dont have ${combinedKey} tag:${debugTag}`);
        }
        ...
        if (this.dataRepairStack.length === 0) {
            if (this.stateIsGood === false) {
                if (this.verboseLogs)
                    this.mainLogger.error(`No active data repair going on tag:${debugTag}`);
            }
            this.stateIsGood = true;
        }
    }
    ...
    repairTrackerClearForNextRepair(repairTracker) {
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + ` repairTrackerClearForNextRepair cycleNumber: ${repairTracker.counter} parition: ${repairTracker.partitionId} `);
    ...
    async mergeAndApplyTXRepairs(cycleNumber, specificParition) {
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair mergeAndApplyTXRepairs cycleNumber ${cycleNumber} partition: ${specificParition}`);
        ...
        for (let partitionID of lastCycleShardValues.ourConsensusPartitions) {
            ...
            for (let tx of repairEntry.newPendingTXs) {
                ...
                if (!keysResponse) {
                    if (this.verboseLogs)
                        this.mainLogger.debug(this.dataPhaseTag + ` _repair mergeAndApplyTXRepairs problem with keysResp  ${utils.stringifyReduce(keysResponse)}  tx:  ${utils.stringifyReduce(tx)}`);
                }
                let { sourceKeys, targetKeys } = keysResponse;
                for (let accountID of sourceKeys) {
            ...
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair mergeAndApplyTXRepairs: extra: ${utils.stringifyReduce(allExtraTXids)}  txIDToAcc: ${utils.stringifyReduce(txIDToAcc)}`);
            ...
            if (txList) {
                for (let i = 0; i < txList.txs.length; i++) {
                    ...
                    if (allExtraTXids[tx.id]) {
                        ...
                        if (!keysResponse) {
                            if (this.verboseLogs)
                                this.mainLogger.debug(this.dataPhaseTag + ` _repair mergeAndApplyTXRepairs problem with keysResp2  ${utils.stringifyReduce(keysResponse)}  tx:  ${utils.stringifyReduce(tx)}`);
                        }
                        let { sourceKeys, targetKeys } = keysResponse;
                        for (let accountID of sourceKeys) {
                            allAccountsToResetById[accountID] = 1;
                ...
                if (this.verboseLogs)
                    this.mainLogger.debug(this.dataPhaseTag + ` _repair mergeAndApplyTXRepairs txList not found for: cycle: ${cycleNumber} in ${utils.stringifyReduce(this.txByCycleByPartition)}`);
            }
            // build and sort a list of TXs that we need to apply
            if (this.verboseLogs && this.extendedRepairLogging)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair mergeAndApplyTXRepairs txIDResetExtraCount: ${txIDResetExtraCount} allAccountsToResetById ${utils.stringifyReduce(allAccountsToResetById)}`);
            // reset accounts
            let accountKeys = Object.keys(allAccountsToResetById);
            if (this.verboseLogs && this.extendedRepairLogging)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair mergeAndApplyTXRepairs revert accountKeys ${utils.stringifyReduce(accountKeys)}`);
            if (this.verboseLogs && this.extendedRepairLogging)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair mergeAndApplyTXRepairs FIFO lock outer: ${cycleNumber}   ${utils.stringifyReduce(accountKeys)}`);
            let ourAccountLocks = await this.bulkFifoLockAccounts(accountKeys);
            if (this.verboseLogs && this.extendedRepairLogging)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair mergeAndApplyTXRepairs FIFO lock inner: ${cycleNumber}   ${utils.stringifyReduce(accountKeys)}`);
            ...
            if (this.verboseLogs && this.extendedRepairLogging)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair mergeAndApplyTXRepairs newTXList ${utils.stringifyReduce(newTXList)}`);
            if (this.verboseLogs && this.extendedRepairLogging)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair mergeAndApplyTXRepairs newTXList.length: ${newTXList.length} txKeys.length: ${txKeys.length} txIDToAccCount: ${txIDToAccCount}`);
            let applyCount = 0;
            ...
            for (let tx of newTXList) {
                ...
                try {
                    if (keysFilter) {
                        ...
                        if (this.verboseLogs)
                            this.mainLogger.debug(this.dataPhaseTag + ` _repair mergeAndApplyTXRepairs apply tx ${utils.makeShortHash(tx.id)} ${tx.timestamp} data: ${utils.stringifyReduce(tx)} with filter: ${utils.stringifyReduce(acountsFilter)}`);
                        ...
                        if (!success) {
                            if (this.verboseLogs)
                                this.mainLogger.debug(this.dataPhaseTag + ' testAccountTime failed. calling apoptosis. mergeAndApplyTXRepairs' + utils.stringifyReduce(tx));
                        ...
                        if (!applied) {
                            applyFailCount++;
                            if (this.verboseLogs)
                                this.mainLogger.debug(this.dataPhaseTag + ` _repair mergeAndApplyTXRepairs apply failed`);
                        }
                        else {
                            applyCount++;
                        }
                        ...
                        if (this.verboseLogs)
                            this.mainLogger.debug(this.dataPhaseTag + ` _repair mergeAndApplyTXRepairs no for ${tx.id} in ${utils.stringifyReduce(txIDToAcc)}`);
                    }
                }
                catch (ex) {
                ...
                if (this.verboseLogs)
                    this.mainLogger.debug(this.dataPhaseTag + ` _repair mergeAndApplyTXRepairs applyCount ${applyCount} applyFailCount: ${applyFailCount}`);
            }
            // unlock the accounts we locked...  todo maybe put this in a finally statement?
            this.bulkFifoUnlockAccounts(accountKeys, ourAccountLocks);
            if (this.verboseLogs && this.extendedRepairLogging)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair mergeAndApplyTXRepairs FIFO unlock: ${cycleNumber}   ${utils.stringifyReduce(accountKeys)}`);
        }
    }
    /**
    ...
    async updateTrackingAndPrepareRepairs(cycleNumber, specificParition) {
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair updateTrackingAndPrepareRepairs cycleNumber ${cycleNumber} partition: ${specificParition}`);
        // walk through all txs for this cycle.
        // get or create entries for accounts.
        // track when they have missing txs or wrong txs
        ...
        try {
            ...
            for (let tx of repairEntry.newPendingTXs) {
                ...
                if (!keysResponse) {
                    if (this.verboseLogs)
                        this.mainLogger.debug(this.dataPhaseTag + ` _repair updateTrackingAndPrepareRepairs problem with keysResp  ${utils.stringifyReduce(keysResponse)}  tx:  ${utils.stringifyReduce(tx)}`);
                }
                let { sourceKeys, targetKeys } = keysResponse;
                for (let accountID of sourceKeys) {
                    allAccountsToResetById[accountID] = 1;
            ...
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair updateTrackingAndPrepareRepairs: extra: ${utils.stringifyReduce(allExtraTXids)}  txIDToAcc: ${utils.stringifyReduce(txIDToAcc)}`);
            // todo repair: hmmm also reset accounts have a tx we need to remove.
            // }
            let txList = this.getTXList(cycleNumber, partitionID); // done todo sharding: pass partition ID
            let txIDToAccCount = 0;
            ...
            if (txList) {
                for (let i = 0; i < txList.txs.length; i++) {
                    ...
                    if (allExtraTXids[tx.id]) {
                        ...
                        if (!keysResponse) {
                            if (this.verboseLogs)
                                this.mainLogger.debug(this.dataPhaseTag + ` _repair updateTrackingAndPrepareRepairs problem with keysResp2  ${utils.stringifyReduce(keysResponse)}  tx:  ${utils.stringifyReduce(tx)}`);
                        }
                        let { sourceKeys, targetKeys } = keysResponse;
                        for (let accountID of sourceKeys) {
                            allAccountsToResetById[accountID] = 1;
                ...
                if (this.verboseLogs && this.extendedRepairLogging)
                    this.mainLogger.debug(this.dataPhaseTag + ` _repair updateTrackingAndPrepareRepairs txIDResetExtraCount:${txIDResetExtraCount} txIDToAccCount: ${txIDToAccCount}`);
            }
            else {
                if (this.verboseLogs)
                    this.mainLogger.debug(this.dataPhaseTag + ` _repair updateTrackingAndPrepareRepairs txList not found for: cycle: ${cycleNumber} in ${utils.stringifyReduce(this.txByCycleByPartition)}`);
            }
            // build and sort a list of TXs that we need to apply
            ...
            newTXList.sort(function (a, b) { return a.timestamp - b.timestamp; });
            if (this.verboseLogs && this.extendedRepairLogging)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair updateTrackingAndPrepareRepairs newTXList ${utils.stringifyReduce(newTXList)}`);
            if (this.verboseLogs && this.extendedRepairLogging)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair updateTrackingAndPrepareRepairs newTXList.length: ${newTXList.length} txKeys.length: ${txKeys.length} txIDToAccCount: ${txIDToAccCount}`);
            ...
            // }
            if (this.verboseLogs && this.extendedRepairLogging)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair updateTrackingAndPrepareRepairs finished`);
    ...
    async applyAllPreparedRepairs(cycleNumber) {
        ...
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair applyAllPreparedRepairs cycleNumber ${cycleNumber}`);
        ...
        // build and sort a list of TXs that we need to apply
        if (this.verboseLogs && this.extendedRepairLogging)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair applyAllPreparedRepairs allAccountsToResetById ${utils.stringifyReduce(allAccountsToResetById)}`);
        // reset accounts
        let accountKeys = Object.keys(allAccountsToResetById);
        if (this.verboseLogs && this.extendedRepairLogging)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair applyAllPreparedRepairs revert accountKeys ${utils.stringifyReduce(accountKeys)}`);
        if (this.verboseLogs && this.extendedRepairLogging)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair applyAllPreparedRepairs FIFO lock outer: ${cycleNumber}   ${utils.stringifyReduce(accountKeys)}`);
        let ourAccountLocks = await this.bulkFifoLockAccounts(accountKeys);
        if (this.verboseLogs && this.extendedRepairLogging)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair applyAllPreparedRepairs FIFO lock inner: ${cycleNumber}   ${utils.stringifyReduce(accountKeys)}`);
        // let replacmentAccounts =  //returned by the below function for debug
        await this._revertAccounts(accountKeys, cycleNumber);
        if (this.verboseLogs && this.extendedRepairLogging)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair applyAllPreparedRepairs newTXList ${utils.stringifyReduce(newTXList)}`);
        if (this.verboseLogs && this.extendedRepairLogging)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair applyAllPreparedRepairs newTXList.length: ${newTXList.length}`);
        let applyCount = 0;
        let applyFailCount = 0;
        let hasEffect = false;
        let accountValuesByKey = {};
        ...
        for (let tx of newTXList) {
            ...
            try {
                if (keysFilter) {
                    ...
                    if (this.verboseLogs)
                        this.mainLogger.debug(this.dataPhaseTag + ` _repair applyAllPreparedRepairs apply tx ${utils.makeShortHash(tx.id)} ${tx.timestamp} data: ${utils.stringifyReduce(tx)} with filter: ${utils.stringifyReduce(acountsFilter)}`);
                    let hasStateTableData = false; // may or may not have it but not tracking yet
                    // HACK!!  receipts sent across the net to us may need to get re parsed
                    if (utils.isString(tx.data.receipt)) {
                        tx.data.receipt = JSON.parse(tx.data.receipt);
                    ...
                    if (!success) {
                        if (this.verboseLogs)
                            this.mainLogger.debug(this.dataPhaseTag + ' applyAllPreparedRepairs testAccountTime failed. calling apoptosis. applyAllPreparedRepairs' + utils.stringifyReduce(tx));
                    ...
                    if (!applied) {
                        applyFailCount++;
                        if (this.verboseLogs)
                            this.mainLogger.debug(this.dataPhaseTag + ` _repair applyAllPreparedRepairs apply failed`);
                    }
                    else {
                        applyCount++;
                    }
                    ...
                    if (this.verboseLogs)
                        this.mainLogger.debug(this.dataPhaseTag + ` _repair applyAllPreparedRepairs no for ${tx.id} in ${utils.stringifyReduce(txIDToAcc)}`);
                }
            }
            catch (ex) {
            ...
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair applyAllPreparedRepairs applyCount ${applyCount} applyFailCount: ${applyFailCount}`);
        }
        // unlock the accounts we locked...  todo maybe put this in a finally statement?
        this.bulkFifoUnlockAccounts(accountKeys, ourAccountLocks);
        if (this.verboseLogs && this.extendedRepairLogging)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair applyAllPreparedRepairs FIFO unlock: ${cycleNumber}   ${utils.stringifyReduce(accountKeys)}`);
        // }
    ...
    async _revertAccounts(accountIDs, cycleNumber) {
        ...
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair _revertAccounts start  numAccounts: ${accountIDs.length} repairing cycle:${cycleNumber}`);
        try {
            ...
            if (replacmentAccounts.length > 0) {
                for (let accountData of replacmentAccounts) {
                    ...
                    if (accountData == null || accountData.data == null || accountData.accountId == null) {
                        if (this.verboseLogs)
                            this.mainLogger.error(this.dataPhaseTag + ` _repair _revertAccounts null account data found: ${accountData.accountId} cycle: ${cycleNumber} data: ${utils.stringifyReduce(accountData)}`);
                    }
                    else {
                        // todo overkill
                        if (this.verboseLogs && this.extendedRepairLogging)
                            this.mainLogger.debug(this.dataPhaseTag + ` _repair _revertAccounts reset: ${utils.makeShortHash(accountData.accountId)} ts: ${utils.makeShortHash(accountData.timestamp)} cycle: ${cycleNumber} data: ${utils.stringifyReduce(accountData)}`);
                    }
                }
                ...
                if (this.verboseLogs)
                    this.mainLogger.debug(this.dataPhaseTag + ` _repair _revertAccounts No replacment accounts found!!! cycle <= :${prevCycle}`);
            }
            if (this.verboseLogs && this.extendedRepairLogging)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair _revertAccounts: ${accountIDs.length} replacmentAccounts ${replacmentAccounts.length} repairing cycle:${cycleNumber}`);
            ...
            for (let accountData of replacmentAccounts) {
                ...
                if (accountData.cycleNumber > prevCycle) {
                    if (this.verboseLogs)
                        this.mainLogger.error(this.dataPhaseTag + ` _repair _revertAccounts cycle too new for backup restore: ${accountData.cycleNumber}  cycleNumber:${cycleNumber} timestamp:${accountData.timestamp}`);
                }
            ...
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair _revertAccounts: ${utils.stringifyReduce(debug)}`);
            ...
            if (accountsToDelete.length > 0) {
                if (this.verboseLogs)
                    this.mainLogger.debug(this.dataPhaseTag + ` _repair _revertAccounts delete some accounts ${utils.stringifyReduce(accountsToDelete)}`);
            ...
            for (let txRecord of this.tempTXRecords) {
                ...
                if (!keysResponse) {
                    if (this.verboseLogs)
                        this.mainLogger.debug(this.dataPhaseTag + ` _repair _revertAccounts problem with keysResp  ${utils.stringifyReduce(keysResponse)}  tx:  ${utils.stringifyReduce(txRecord.acceptedTx)}`);
                }
                let { sourceKeys, targetKeys } = keysResponse;
                for (let accountID of sourceKeys) {
                    if (accMap[accountID]) {
    ...
    async broadcastPartitionResults(cycleNumber) {
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair broadcastPartitionResults for cycle: ${cycleNumber}`);
        ...
        for (let nodeId of nodesToTell) {
            ...
            if (this.verboseLogs && this.extendedRepairLogging)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair broadcastPartitionResults to ${nodeId} debugStr: ${partitionResultsToSend.debugStr} res: ${utils.stringifyReduce(payload)}`);
            if (this.verboseLogs && this.extendedRepairLogging)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair broadcastPartitionResults to ${nodeId} debugStr: ${partitionResultsToSend.debugStr} res: ${utils.stringifyReduce(payload)}`);
    ...
    startShardCalculations() {
        ...
        this._registerListener(this.p2p.state, 'cycle_q3_start', async (lastCycle, time) => {
            ...
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair startSyncPartitions:cycle_q3_start cycle: ${lastCycle.counter}`);
    ...
    async startSyncPartitions() {
        ...
        this._registerListener(this.p2p.state, 'cycle_q2_start', async (lastCycle, time) => {
            ...
            if (this.verboseLogs)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair startSyncPartitions:cycle_q2_start cycle: ${lastCycle.counter}`);
    ...
    async updateAccountsCopyTable(accountDataList, repairing, txTimestamp) {
        ...
        if (txTimestamp + this.syncSettleTime < cycleStart) {
            if (this.verboseLogs)
                this.mainLogger.error(this.dataPhaseTag + `updateAccountsCopyTable time error< ts:${txTimestamp} cs:${cycleStart} ce:${cycleEnd} `);
        }
        ...
        if (accountDataList[0].timestamp !== txTimestamp) {
            if (this.verboseLogs)
                this.mainLogger.error(this.dataPhaseTag + `updateAccountsCopyTable timestamps dot match txts:${txTimestamp} acc.ts:${accountDataList[0].timestamp} `);
        }
        ...
        for (let accountEntry of accountDataList) {
            ...
            if (this.verboseLogs && this.extendedRepairLogging)
                this.mainLogger.debug(this.dataPhaseTag + `updateAccountsCopyTable acc.timestamp: ${timestamp} cycle computed:${cycleNumber} accountId:${utils.makeShortHash(accountId)}`);
            // todo perf. batching?
   ...
   */
    ...
    processTempTXs(cycle) {
        ...
        for (let txRecord of this.tempTXRecords) {
            if (txRecord.redacted > 0) {
                if (this.verboseLogs && this.extendedRepairLogging)
                    this.mainLogger.debug(this.dataPhaseTag + ` _repair recordTXByCycle: ${utils.makeShortHash(txRecord.acceptedTx.id)} cycle: ${cycle.counter} redacted!!! ${txRecord.redacted}`);
                continue;
            }
            if (txRecord.txTS < cycleEnd) {
        ...
        if (this.verboseLogs && this.extendedRepairLogging)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair processTempTXs txsRecorded: ${txsRecorded} txsTemp: ${txsTemp} `);
    }
    ...
    recordTXByCycle(txTS, acceptedTx, passed, applyResponse) {
        ...
        for (let accountKey of allKeys) {
            ...
            if (seenParitions[key] != null) {
                if (this.verboseLogs && this.extendedRepairLogging)
                    this.mainLogger.debug(this.dataPhaseTag + ` _repair recordTXByCycle: ${utils.makeShortHash(acceptedTx.id)} cycle: ${cycleNumber} entries: ${txList.hashes.length} --TX already recorded for cycle`);
            ...
            if (this.verboseLogs && this.extendedRepairLogging)
                this.mainLogger.debug(this.dataPhaseTag + ` _repair recordTXByCycle: ${utils.makeShortHash(acceptedTx.id)} cycle: ${cycleNumber} entries: ${txList.hashes.length}`);
        }
    }
    /**
     * getPartitionObject
    ...
    findMostCommonResponse(cycleNumber, partitionId, ignoreList) {
        ...
        if (this.verboseLogs && this.extendedRepairLogging)
            this.mainLogger.debug(this.dataPhaseTag + ` _repair findMostCommonResponse: retVal: ${utils.stringifyReduce({ topHash, topCount, topResult })}  responses: ${utils.stringifyReduce(responses)} `);
    ...
    sendPartitionData(partitionReceipt, paritionObject) {
        ...
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + ' sendPartitionData ' + utils.stringifyReduceLimit({ combinedReciept, paritionObject }));
        // send it
    ...
    sendTransactionData(partitionNumber, cycleNumber, transactions) {
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + ' sendTransactionData ' + utils.stringifyReduceLimit({ partitionNumber, cycleNumber, transactions }));
        // send it
    ...
    trySendAndPurgeReceiptsToArchives(partitionReceipt) {
        ...
        if (this.verboseLogs)
            this.mainLogger.debug(this.dataPhaseTag + ' trySendAndPurgeReceipts ' + key);
