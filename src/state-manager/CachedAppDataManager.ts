...
class CachedAppDataManager {
  ...
  setupHandlers(): void {
      ...
      handler: async (payloadBuffer, respond, header, sign) => {
        ...
        try {
          const requestStream = getStreamWithTypeCheck(
            payloadBuffer,
            TypeIdentifierEnum.cGetCachedAppDataReq
          );
          if (!requestStream) {
    ...
    this.p2p.registerInternalBinary(
      getCachedAppDataBinaryHandler.name,
      getCachedAppDataBinaryHandler.handler
    );
  }
  ...
  factValidateCorrespondingCachedAppDataSender(
    ...
    const allNodes = Array.from(new Set([...senderGroup, ...targetGroup])).sort((a, b) =>
  ...
  factSendCorrespondingCachedAppData(
    ...
    const senderGroup = this.stateManager.transactionQueue.getConsenusGroupForAccount(
      queueEntry.executionShardKey
    );
    const targetGroup = this.stateManager.transactionQueue.getConsenusGroupForAccount(dataID);
    const allNodes = Array.from(new Set([...senderGroup, ...targetGroup])).sort((a, b) =>
      a.id.localeCompare(b.id)
    ...
    if (correspondingNodes.length > 0) {
      ...
      if (this.config.p2p.useBinarySerializedEndpoints && this.config.p2p.sendCachedAppDataBinary) {
        if (logFlags.shardedCache)
          console.log(`cachedAddData: factSendCorrespondingCachedAppData sending ${dataID}`);
