...
class StateManagerSync {
  ...
  async syncStateData (requiredNodeCount: number) {
    ...
    while (hasValidShardData === false) {
      ...
      if(this.stateManager.currentCycleShardData == null){
        if (this.logger.playbackLogEnabled ) this.logger.playbackLogNote('shrd_sync_waitForShardData', ` `, ` ${utils.stringifyReduce(this.stateManager.currentCycleShardData)} `)
      ...
      if(this.stateManager.currentCycleShardData != null){
        if(this.stateManager.currentCycleShardData.hasCompleteData == false){
          ...
          if (this.logger.playbackLogEnabled ) this.logger.playbackLogNote('shrd_sync_waitForShardData', ` `, `hasCompleteData:${this.stateManager.currentCycleShardData.hasCompleteData} active:${utils.stringifyReduce(temp)} ${utils.stringifyReduce(this.stateManager.currentCycleShardData)} `)
    ...
    if (nodeShardData.storedPartitions.rangeIsSplit === true) {
      ...
      console.log(`syncRangeGoal ${syncRangeGoal}  chunksGuide:${chunksGuide} numPartitions:${this.stateManager.currentCycleShardData.shardGlobals.numPartitions} partitionsPerRange:${partitionsPerRange}`)
...
async syncStateTableData (lowAddress: string, highAddress: string, startTime: number, endTime: number) {
  ...
  while (searchingForGoodData) { // todo m11: this needs to be replaced
    ...
    let nodes:Shardus.Node[] = ShardFunctions.getNodesByProximity(this.stateManager.currentCycleShardData.shardGlobals, this.stateManager.currentCycleShardData.activeNodes, centerNode.ourNodeIndex, this.p2p.id, 40)
 ...
 */
    ...
    async syncStateDataFast (requiredNodeCount: number) {
        ...
        this.mainLogger.debug(`DATASYNC: starting syncStateDataFast`)
      ...
      async syncAccountDataFast (lowAddress:string, highAddress:string) {
        ...
        while (moreDataRemaining) {
          ...
          if(this.dataSourceNode == null || this.stateManager.isNodeValidForInternalMessage(this.dataSourceNode.id, "syncAccountData", true, true) === false){
            if (this.verboseLogs && this.dataSourceNode == null) { this.mainLogger.error(`syncAccountDataFast   this.dataSourceNode == null`) }
            if(this.tryNextDataSourceNode('syncAccountData') == false){
              break
            }
            continue
