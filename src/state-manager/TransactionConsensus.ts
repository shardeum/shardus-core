...
class TransactionConsenus {
    ...
    Context.network.registerExternalGet(
      ...
      (_req, res) => {
        try {
          ...
          res.write(
            `stateManager.waitTimeBeforeConfirm: ${this.config.stateManager.waitTimeBeforeConfirm}\n`
          );
        } catch (e) {
          res.write(`${e}\n`);
        }
    ...
    Context.network.registerExternalGet(
      ...
      (_req, res) => {
        try {
          ...
          res.write(
            `stateManager.waitTimeBeforeReceipt: ${this.config.stateManager.waitTimeBeforeReceipt}\n`
          );
        } catch (e) {
          res.write(`${e}\n`);
        }
      ...
      handler: async (payload, respond, header) => {
        ...
        try {
          ...
          if (
            ...
            nestedCountersInstance.countEvent(
              'consensus',
              'get_tx_timestamp found tx timestamp in cacheById'
            );
            return respond(tsReceipt, serializeGetTxTimestampResp);
          } else {
            const tsReceipt: Shardus.TimestampReceipt = this.generateTimestampReceipt(
      ...
      async (payload: any, tracker: string, msgSize: number) => {
        ...
        try {
          ...
          if (
            ...
            receivedAppliedReceipt2.confirmOrChallenge == null
          ) {
            /* prettier-ignore */ this.mainLogger.error(`spread_appliedReceipt2 ${txId} received null receipt`)
            nestedCountersInstance.countEvent(
              `consensus`,
              `spread_appliedReceipt received null confirm message`
            );
            return;
          }
    ...
    Comms.registerGossipHandler(
      ...
      (payload: AppliedReceipt2 & { txGroupCycle: number }) => {
        ...
        try {
          ...
          if (logFlags.verbose)
            this.mainLogger.debug(
              `POQo: received receipt from gossip for ${queueEntry.logID} forwarding gossip`
            );
          ...
          if (!hasTwoThirdsMajority) {
            ...
            nestedCountersInstance.countEvent(
              'poqo',
              'poqo-receipt-gossip: Rejecting receipt because no majority'
            );
            return;
          }
    ...
    const poqoDataAndReceiptBinaryHandler: Route<InternalBinaryHandler<Buffer>> = {
      ...
      handler: async (payload, respond, header, sign) => {
        ...
        try {
          ...
          const queueEntry = this.stateManager.transactionQueue.getQueueEntrySafe(
  ...
  async poqoVoteSendLoop(queueEntry: QueueEntry, appliedVoteHash: AppliedVoteHash): Promise<void> {
    ...
    while (!queueEntry.poqoReceipt) {
      ...
      nestedCountersInstance.countEvent(
        'poqo',
        `At index ${queueEntry.poqoNextSendIndex} in poqoVoteSendLoop`
      );
      ...
      if (queueEntry.acceptVoteMessage === false || queueEntry.appliedReceipt2 != null) {
        if (queueEntry.acceptVoteMessage === false)
          if (logFlags.seqdiagram)
            /* prettier-ignore */ this.seqLogger.info(`0x53455103 ${shardusGetTime()} tx:${queueEntry.acceptedTx.txId} Note over ${NodeList.activeIdToPartition.get(Self.id)}: gossipHandlerAV:f no_accept`)
        if (queueEntry.appliedReceipt2 != null)
          if (logFlags.seqdiagram)
            /* prettier-ignore */ this.seqLogger.info(`0x53455103 ${shardusGetTime()} tx:${queueEntry.acceptedTx.txId} Note over ${NodeList.activeIdToPartition.get(Self.id)}: gossipHandlerAV:f applied2_not_null`)
        return false;
      }
      /* prettier-ignore */ if (logFlags.playback) this.logger.playbackLogNote('tryAppendVote', `${queueEntry.logID}`, `vote: ${utils.stringifyReduce(vote)}`)
      /* prettier-ignore */ if (logFlags.debug) this.mainLogger.debug(`tryAppendVote collectedVotes: ${queueEntry.logID}   vote: ${utils.stringifyReduce(vote)}`)
