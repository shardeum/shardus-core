...
} from '../types/RequestStateForTxReq';
import {
...
class TransactionQueue {
          ...
          for (const data of payload.stateList) {
            if (
              ...
              if (coveredByUs) {
                ...
                if (logFlags.verbose)
                  console.log(`broadcast_state: coveredByUs: ${data.accountId} no need to accept this data`);
                continue;
              } else {
            ...
            if (queueEntry.state === 'syncing') {
              if (logFlags.playback)
                this.logger.playbackLogNote(
                  'shrd_sync_gotBroadcastData',
                  `${queueEntry.acceptedTx.txId}`,
                  ` qId: ${queueEntry.entryID} data:${data.accountId}`
                );
            }
          }
        } finally {
          ...
          for (let i = 0; i < req.stateList.length; i++) {
            ...
            if (
              ...
              if (coveredByUs) {
                nestedCountersInstance.countEvent('processing', 'broadcast_state_coveredByUs');
                if (logFlags.verbose)
                  console.log(`broadcast_state: coveredByUs: ${state.accountId} no need to accept this data`);
                continue;
              } else {
            ...
            if (queueEntry.state === 'syncing') {
              if (logFlags.playback)
                this.logger.playbackLogNote(
                  'shrd_sync_gotBroadcastData',
                  `${queueEntry.acceptedTx.txId}`,
                  ` qId: ${queueEntry.entryID} data:${state.accountId}`
                );
            }
          }
        } catch (e) {
        ...
        try {
          ...
          if (logFlags.debug)
            this.mainLogger.debug(
              `broadcast_finalstate ${queueEntry.logID}, ${Utils.safeStringify(payload.stateList)}`
            );
          // add the data in
          ...
          for (const data of payload.stateList) {
            ...
            if (_sender == null || _sender.id == null) {
              if (logFlags.error)
                this.mainLogger.error(
                  `broadcast_finalstate invalid sender for data: ${data.accountId}, sender: ${JSON.stringify(_sender)}`
                );
              continue;
            }
            const isValidFinalDataSender = this.factValidateCorrespondingTellFinalDataSender(
              queueEntry,
              _sender.id
            );
            if (isValidFinalDataSender === false) {
              if (logFlags.error)
                this.mainLogger.error(
                  `broadcast_finalstate invalid sender ${_sender.id} for data: ${data.accountId}`
                );
              continue;
            }
            if (queueEntry.collectedFinalData[data.accountId] == null) {
              ...
              if (logFlags.playback && logFlags.verbose)
                this.logger.playbackLogNote(
                  'broadcast_finalstate',
                  `${queueEntry.logID}`,
                  `broadcast_finalstate addFinalData qId: ${queueEntry.entryID} data:${utils.makeShortHash(data.accountId)} collected keys: ${utils.stringifyReduce(Object.keys(queueEntry.collectedFinalData))}`
                );
            }
    ...
    const broadcastFinalStateRoute: P2PTypes.P2PTypes.Route<InternalBinaryHandler<Buffer>> = {
      ...
      handler: (payload, response, header, sign) => {
        ...
        try {
          ...
          if (logFlags.verbose && logFlags.console)
            console.log(`${route}: txId: ${req.txid} stateSize: ${req.stateList.length}`);
          ...
          for (const data of req.stateList) {
            ...
            if (isValidFinalDataSender === false) {
              if (logFlags.error)
                this.mainLogger.error(
                  `broadcast_finalstate invalid sender ${header.sender_id} for data: ${data.accountId}`
                );
              continue;
            }
            if (queueEntry.collectedFinalData[data.accountId] == null) {
      ...
      handler: async (payload, respond, header, sign) => {
        ...
        try {
          const requestStream = getStreamWithTypeCheck(
            payload,
            TypeIdentifierEnum.cSpreadTxToGroupSyncingReq
          );
          if (!requestStream) {
            return errorHandler(RequestErrorEnum.InvalidRequest);
          }
        ...
        try {
          ...
          if (payload.txGroupCycle) {
            if (queueEntry.txGroupCycle !== payload.txGroupCycle) {
              if (logFlags.error)
                this.mainLogger.error(
                  `gossip-final-state mismatch txGroupCycle for txid: ${payload.txid}, sender's txGroupCycle: ${payload.txGroupCycle}, our txGroupCycle: ${queueEntry.txGroupCycle}`
                );
              nestedCountersInstance.countEvent(
                'processing',
          ...
          if (logFlags.debug)
            this.mainLogger.debug(
              `gossip-final-state ${queueEntry.logID}, ${Utils.safeStringify(payload.stateList)}`
            );
          // add the data in
          let saveSomething = false;
          for (const data of payload.stateList) {
          ...
          if (saveSomething) {
            ...
            for (const data of payload.stateList) {
              ...
              const storageNodes = this.stateManager.transactionQueue.getStorageGroupForAccount(
                data.accountId
              );
              for (const node of storageNodes) {
                nodesToSendTo.add(node);
  ...
  addTransactionToNonceQueue(nonceQueueEntry: NonceQueueItem): {
    ...
    try {
      ...
      if (logFlags.seqdiagram)
        this.seqLogger.info(
          `0x53455106 ${shardusGetTime()} tx:${nonceQueueEntry.txId} Note over ${NodeList.activeIdToPartition.get(Self.id)}: pause_nonceQ`
        );
  ...
  handleSharedTX(tx: Shardus.TimestampedTx, appData: unknown, sender: Shardus.Node): QueueEntry {
    ...
    if (inRangeOfCurrentTime(timestamp, mostOfQueueSitTimeMs, txExpireTimeMs) === false) {
      ...
      if (logFlags.playback)
        this.logger.playbackLogNote(
          'shrd_spread_tx_to_groupToOldOrTooFuture',
          '',
          'spread_tx_to_group working on tx with age: ' + age
        );
      profilerInstance.profileSectionEnd('handleSharedTX');
      return null;
    }
    ...
    if (queueEntry == null) {
      ...
      if (logFlags.playback)
        this.logger.playbackLogNote(
          'spread_tx_to_group_noQE',
          '',
          `spread_tx_to_group failed: cant find queueEntry for:  ${utils.makeShortHash(id)}`
        );
      profilerInstance.profileSectionEnd('handleSharedTX');
      return null;
    }
  ...
  async preApplyTransaction(queueEntry: QueueEntry): Promise<PreApplyAcceptedTransactionResult> {
    ...
    if (logFlags.verbose)
      if (logFlags.console) console.log('preApplyTransaction ' + timestamp + ' debugInfo:' + debugInfo);
    if (logFlags.verbose)
      this.mainLogger.debug('preApplyTransaction ' + timestamp + ' debugInfo:' + debugInfo);
    ...
    for (const key of uniqueKeys) {
      if (wrappedStates[key] == null) {
        if (logFlags.verbose)
          if (logFlags.console)
            console.log(
              `preApplyTransaction missing some account data. timestamp:${timestamp}  key: ${utils.makeShortHash(key)}  debuginfo:${debugInfo}`
            );
    ...
    if (!accountTimestampsAreOK) {
      ...
      if (logFlags.playback)
        this.logger.playbackLogNote(
          'tx_preapply_rejected 1',
          `${acceptedTX.txId}`,
          `Transaction: ${utils.stringifyReduce(acceptedTX)}`
        );
      this.txDebugMarkEndTime(queueEntry, 'preApplyTransaction');
      return {
        applied: false,
    ...
    try {
      ...
      if (logFlags.verbose && this.stateManager.extendedRepairLogging)
        this.mainLogger.debug(` preApplyTransaction FIFO lock outer: ${utils.stringifyReduce(uniqueKeys)} `);
      ourAccountLocks = await this.stateManager.bulkFifoLockAccounts(uniqueKeys);
      if (logFlags.verbose && this.stateManager.extendedRepairLogging)
        this.mainLogger.debug(
          ` preApplyTransaction FIFO lock inner: ${utils.stringifyReduce(uniqueKeys)} ourLocks: ${utils.stringifyReduce(ourAccountLocks)}`
        );
      ...
      this.setDebugLastAwaitedCallInner(
        'preApplyTransaction-fifoLock(accountModification)',
      ...
      if (configContext.stateManager.useCopiedWrappedStateForApply === true) {
        ...
        this.setDebugLastAwaitedCallInner(
          'stateManager.transactionQueue.app.apply(tx)',
          DebugComplete.Completed
        );
      } else {
        this.setDebugLastAwaitedCallInner('stateManager.transactionQueue.app.apply(tx)');
        ...
        for (const account of applyResponse.accountWrites) {
          ...
          for (const storageNode of homeNode.nodeThatStoreOurParitionFull) {
            ...
            if (!isStorageNodeInTxGroup) {
              ...
              if (logFlags.verbose)
                this.mainLogger.debug(
                  `preApplyTransaction Storage node ${storageNode.id} of accountId ${account.accountId} is not in transaction group`
                );
              break;
            }
          }
          if (logFlags.verbose)
            this.mainLogger.debug(
              `preApplyTransaction isUnexpectedAccountWrite for account ${account.accountId}`,
              isUnexpectedAccountWrite
            );
      ...
      if (logFlags.verbose)
        this.mainLogger.debug(
          `preApplyTransaction ${queueEntry.logID} post apply wrappedStates: ${utils.stringifyReduce(wrappedStates)}`
        );
      if (this.stateManager.consensusLog)
        this.mainLogger.debug(`preApplyTransaction ${queueEntry.logID} completed.`);

      //applyResponse = queueEntry?.preApplyTXResult?.applyResponse
      //super verbose option:

      if (logFlags.verbose && applyResponse != null)
        this.mainLogger.debug(
          `preApplyTransaction ${queueEntry.logID}  post applyResponse: ${utils.stringifyReduce(applyResponse)}`
        );
    } catch (ex) {
      if (logFlags.error)
        this.mainLogger.error(
          `preApplyTransaction failed id:${utils.makeShortHash(acceptedTX.txId)}: ` +
            ex.name +
            ': ' +
            ex.message +
            ' at ' +
      ...
      if (logFlags.error)
        this.mainLogger.error(
          `preApplyTransaction failed id:${utils.makeShortHash(acceptedTX.txId)}  ${utils.stringifyReduce(acceptedTX)}`
        );
      ...
      if (logFlags.verbose)
        this.mainLogger.debug(
          ` preApplyTransaction FIFO unlock inner: ${utils.stringifyReduce(uniqueKeys)} ourLocks: ${utils.stringifyReduce(ourAccountLocks)}`
        );
    }

    if (logFlags.playback)
      this.logger.playbackLogNote('tx_preapplied', `${acceptedTX.txId}`, `preApplyTransaction ${timestamp} `);
  ...
  async commitConsensedTransaction(queueEntry: QueueEntry): Promise<CommitConsensedTransactionResult> {
    ...
    try {
      ...
      if (logFlags.verbose) {
        this.mainLogger.debug(
          `commitConsensedTransaction txId: ${queueEntry.logID}  ts:${timestamp} isGlobalModifyingTX:${isGlobalModifyingTX}  Applying! debugInfo: ${debugInfo}`
        );
        this.mainLogger.debug(
        ...
        this.mainLogger.debug(
          `commitConsensedTransaction  preApplyResponse: ${utils.stringifyReduce(queueEntry.preApplyTXResult.applyResponse)}`
        );
      ...
      if (logFlags.verbose && this.stateManager.extendedRepairLogging)
        this.mainLogger.debug(
          `commitConsensedTransaction FIFO lock outer: ${utils.stringifyReduce(uniqueKeys)} `
        );
      ...
      ourAccountLocks = await this.stateManager.bulkFifoLockAccounts(uniqueKeys);
      this.setDebugLastAwaitedCallInner(
        'commit this.stateManager.bulkFifoLockAccounts',
        DebugComplete.Completed
      );
      if (logFlags.verbose && this.stateManager.extendedRepairLogging)
        this.mainLogger.debug(
          `commitConsensedTransaction FIFO lock inner: ${utils.stringifyReduce(uniqueKeys)} ourLocks: ${utils.stringifyReduce(ourAccountLocks)}`
        );
      ...
      if (logFlags.verbose)
        if (logFlags.console)
          console.log(`commitConsensedTransaction tx:${queueEntry.logID} ts:${timestamp} Applying!`);
      ...
      if (logFlags.verbose)
        this.mainLogger.debug(
          `commitConsensedTransaction tx:${queueEntry.logID} ts:${timestamp} accounts: ${utils.stringifyReduce(Object.keys(wrappedStates))}  `
        );
      ...
      if (
        ...
        if (logFlags.verbose)
          this.mainLogger.debug(
            `commitConsensedTransaction applyResponse.accountWrites tx:${queueEntry.logID} ts:${timestamp} accounts: ${utils.stringifyReduce(Object.keys(wrappedStates))}  `
          );
      }
      ...
      if (
        ...
        for (const key of Object.keys(queueEntry.collectedFinalData)) {
          ...
          if (logFlags.verbose)
            this.mainLogger.debug(
              `commitConsensedTransaction collectedFinalData tx:${queueEntry.logID} ts:${timestamp} ${utils.makeShortHash(finalAccount)} preveStateID: ${finalAccount.prevStateId} vs expected: ${prevStateCalc}`
            );
        ...
        if (logFlags.verbose)
          this.mainLogger.debug(
            `commitConsensedTransaction collectedFinalData tx:${queueEntry.logID} ts:${timestamp} accounts: ${utils.stringifyReduce(Object.keys(wrappedStates))}  `
          );
      }
      ...
      if (
        ...
        for (const writtenAccount of applyResponse.accountWrites) {
          ...
          if (logFlags.verbose)
            this.mainLogger.debug(
              `commitConsensedTransaction  tx:${queueEntry.logID} ts:${timestamp} setWritefilter: ${isLocal}  acc:${writtenAccount.accountId} `
            );
        }
      }
      ...
      if (this.executeInOneShard && applyResponse == null && queueEntry.collectedFinalData != null) {
        for (const writtenAccount of Object.values(wrappedStates)) {
          ...
          if (logFlags.verbose)
            this.mainLogger.debug(
              `commitConsensedTransaction queueEntry.collectedFinalData != null tx:${queueEntry.logID} ts:${timestamp} setWritefilter: ${isLocal}  acc:${writtenAccount.accountId} `
            );
        }
      }
      ...
      if (logFlags.verbose)
        this.mainLogger.debug(
          `commitConsensedTransaction  post apply wrappedStates: ${utils.stringifyReduce(wrappedStates)}`
        );
      ...
      if (logFlags.debug)
        this.mainLogger.debug(
          `commitConsensedTransaction failed id:${utils.makeShortHash(acceptedTX.txId)}  ${utils.stringifyReduce(acceptedTX)}`
        );
      ...
      if (logFlags.verbose)
        this.mainLogger.debug(
          `commitConsensedTransaction FIFO unlock inner: ${utils.stringifyReduce(uniqueKeys)} ourLocks: ${utils.stringifyReduce(ourAccountLocks)}`
        );
    }
    ...
    for (const wrappedData of applyResponse.accountData) {
      ...
      if (queueData != null) {
        ...
        if (logFlags.error)
          this.mainLogger.error(
            `commitConsensedTransaction failed to get account data for stats ${wrappedData.accountId}`
          );
      }
    }
  ...
  updateHomeInformation(txQueueEntry: QueueEntry): void {
    ...
    if (cycleShardData != null && txQueueEntry.hasShardInfo === false) {
      ...
      for (const key of txQueueEntry.txKeys.allKeys) {
        ...
        if (homeNode == null) {
          if (logFlags.verbose)
            if (logFlags.error) this.mainLogger.error(` routeAndQueueAcceptedTransaction: ${key} `);
  ...
  routeAndQueueAcceptedTransaction(
    ...
    if (logFlags.playback)
      this.logger.playbackLogNote(
        'routeAndQueueAcceptedTransaction-debug',
        '',
        `sendGossip:${sendGossip} globalModification:${globalModification} noConsensus:${noConsensus} this.readyforTXs:${this.stateManager.accountSync.readyforTXs} hasshardData:${this.stateManager.currentCycleShardData != null} acceptedTx:${utils.stringifyReduce(acceptedTx)} `
      );
    if (this.stateManager.accountSync.readyforTXs === false) {
      if (logFlags.verbose)
        if (logFlags.error)
          this.mainLogger.error(
            `routeAndQueueAcceptedTransaction too early for TX: this.readyforTXs === false`
          );
      return 'notReady'; // it is too early to care about the tx
    }
    if (this.stateManager.currentCycleShardData == null) {
      if (logFlags.verbose)
        if (logFlags.error)
          this.mainLogger.error(
            `routeAndQueueAcceptedTransaction too early for TX: this.stateManager.currentCycleShardData == null`
          );
      return 'notReady';
    }
    ...
    try {
      ...
      if (this.stateManager.accountGlobals.hasknownGlobals == false) {
        if (logFlags.verbose)
          if (logFlags.error)
            this.mainLogger.error(
              `routeAndQueueAcceptedTransaction too early for TX: hasknownGlobals == false`
            );
        return 'notReady';
      }
      ...
      if (Context.config.stateManager.deterministicTXCycleEnabled) {
        ...
        if (cycleNumber > this.stateManager.currentCycleShardData.cycleNumber) {
          if (logFlags.error)
            this.mainLogger.error(
              `routeAndQueueAcceptedTransaction derived txGroupCycle > currentCycleShardData.cycleNumber. txId:${txId} txGroupCycle:${cycleNumber} currentCycleShardData.cycleNumber:${this.stateManager.currentCycleShardData.cycleNumber}`
            );
          nestedCountersInstance.countEvent(
            'stateManager',
            'derived txGroupCycle is larger than current cycle'
          );
          ...
          nestedCountersInstance.countEvent(
            'stateManager',
            'derived txGroupCycle is less than current cycle'
          );
        } else if (cycleNumber === this.stateManager.currentCycleShardData.cycleNumber) {
      ...
      try {
        ...
        for (const key of txQueueEntry.txKeys.allKeys) {
          if (key == null) {
            ...
            if (logFlags.verbose)
              if (logFlags.error)
                this.mainLogger.error(
                  `routeAndQueueAcceptedTransaction key == null ${timestamp} not putting tx in queue.`
                );
            return false;
          }
        ...
        if (txQueueEntry.txKeys.allKeys == null || txQueueEntry.txKeys.allKeys.length === 0) {
          if (logFlags.verbose)
            if (logFlags.error)
              this.mainLogger.error(
                `routeAndQueueAcceptedTransaction allKeys == null || allKeys.length === 0 ${timestamp} not putting tx in queue.`
              );
          return false;
        }
        let cycleShardData = this.stateManager.currentCycleShardData;
        ...
        if (cycleShardData == null) {
          if (logFlags.error)
            this.mainLogger.error(
              `routeAndQueueAcceptedTransaction logID:${txQueueEntry.logID} cycleShardData == null cycle:${cycleNumber} not putting tx in queue.`
            );
          nestedCountersInstance.countEvent(
            'stateManager',
            'routeAndQueueAcceptedTransaction cycleShardData == null'
          );
          return false;
        }
        ...
        if (txQueueEntry.globalModification === false && this.executeInOneShard) {
          ...
          if (logFlags.verbose)
            this.mainLogger.debug(
              `routeAndQueueAcceptedTransaction set executionShardKey tx:${txQueueEntry.logID} ts:${timestamp} executionShardKey: ${utils.stringifyReduce(txQueueEntry.executionShardKey)}  `
            );
          ...
          for (let idx = 0; idx < txQueueEntry.executionGroup.length; idx++) {
            ...
            if (node.id === ourID) {
              ...
              if (logFlags.seqdiagram)
                this.seqLogger.info(
                  `0x53455105 ${shardusGetTime()} tx:${txId} Note over ${NodeList.activeIdToPartition.get(Self.id)}: executor index ${txQueueEntry.ourExGroupIndex}:${(node as Shardus.NodeWithRank).rank}`
                );
            }
          }
          if (txQueueEntry.eligibleNodeIdsToConfirm.has(Self.id)) {
            if (logFlags.seqdiagram)
              this.seqLogger.info(
                `0x53455105 ${shardusGetTime()} tx:${txId} Note over ${NodeList.activeIdToPartition.get(Self.id)}: confirmator`
              );
          }
          if (txQueueEntry.eligibleNodeIdsToVote.has(Self.id)) {
            if (logFlags.seqdiagram)
              this.seqLogger.info(
                `0x53455105 ${shardusGetTime()} tx:${txId} Note over ${NodeList.activeIdToPartition.get(Self.id)}: voter`
              );
          }
          if (logFlags.seqdiagram)
            this.seqLogger.info(
              `0x53455105 ${shardusGetTime()} tx:${txId} Note over ${NodeList.activeIdToPartition.get(Self.id)}: groupsize voters ${txQueueEntry.eligibleNodeIdsToConfirm.size}`
            );
          if (logFlags.seqdiagram)
            this.seqLogger.info(
              `0x53455105 ${shardusGetTime()} tx:${txId} Note over ${NodeList.activeIdToPartition.get(Self.id)}: groupsize confirmators ${txQueueEntry.eligibleNodeIdsToConfirm.size}`
            );
          if (logFlags.seqdiagram)
            this.seqLogger.info(
              `0x53455105 ${shardusGetTime()} tx:${txId} Note over ${NodeList.activeIdToPartition.get(Self.id)}: groupsize execution ${txQueueEntry.executionGroup.length}`
            );
          ...
          if (logFlags.verbose)
            this.mainLogger.debug(
              `routeAndQueueAcceptedTransaction info ${txQueueEntry.logID} isInExecutionHome:${txQueueEntry.isInExecutionHome} hasShardInfo:${txQueueEntry.hasShardInfo}`
            );
          if (logFlags.playback)
            this.logger.playbackLogNote(
              'routeAndQueueAcceptedTransaction',
          ...
          if (this.stateManager.consensusLog)
            this.mainLogger.debug(
              `routeAndQueueAcceptedTransaction info ${txQueueEntry.logID} isInExecutionHome:${txQueueEntry.isInExecutionHome}`
            );
        }
        ...
        if (txQueueEntry.cycleToRecordOn < 0) {
          ...
          if (logFlags.verbose)
            if (logFlags.error)
              this.mainLogger.error(
                `routeAndQueueAcceptedTransaction failed to calculate cycle ${timestamp} error code:${txQueueEntry.cycleToRecordOn}`
              );
          return false;
        }
        ...
        if (logFlags.playback)
          this.logger.playbackLogNote(
            'shrd_queueInsertion_start',
            txQueueEntry.logID,
            `${txQueueEntry.logID} uniqueKeys:${utils.stringifyReduce(txQueueEntry.uniqueKeys)}  txKeys: ${utils.stringifyReduce(txQueueEntry.txKeys)} cycleToRecordOn:${txQueueEntry.cycleToRecordOn}`
          );
          ...
          if (globalModification === true) {
            if (this.stateManager.accountGlobals.isGlobalAccount(key)) {
              if (logFlags.playback)
                this.logger.playbackLogNote(
                  'globalAccountMap',
                  `routeAndQueueAcceptedTransaction - has account:${utils.stringifyReduce(key)}`
                );
            } else {
              ...
              if (logFlags.playback)
                this.logger.playbackLogNote(
                  'globalAccountMap',
                  `routeAndQueueAcceptedTransaction - set account:${utils.stringifyReduce(key)}`
                );
            }
          }
        }
        ...
        if (age > this.stateManager.queueSitTime * 0.9) {
          if (txQueueEntry.didSync === true) {
            nestedCountersInstance.countEvent(
              'stateManager',
              `enqueue old TX didSync === true queuedBeforeMainSyncComplete:${txQueueEntry.queuedBeforeMainSyncComplete}`
            );
          } else {
            nestedCountersInstance.countEvent(
              'stateManager',
              `enqueue old TX didSync === false queuedBeforeMainSyncComplete:${txQueueEntry.queuedBeforeMainSyncComplete}`
            );
        ...
        if (txQueueEntry.hasShardInfo) {
          ...
          if (logFlags.seqdiagram)
            this.seqLogger.info(
              `0x53455105 ${shardusGetTime()} tx:${txId} Note over ${NodeList.activeIdToPartition.get(Self.id)}: groupsize transaction ${txQueueEntry.transactionGroup.length}`
            );
          if (txQueueEntry.ourNodeInTransactionGroup || txQueueEntry.didSync === true) {
            ...
            this.mainLogger.debug(
              `routeAndQueueAcceptedTransaction isSenderWrappedTxGroup ${txQueueEntry.logID} ${utils.stringifyReduce(txQueueEntry.isSenderWrappedTxGroup)}`
            );
          }
          ...
          if (txQueueEntry.didSync === false) {
            ...
            if (
              ...
              if (logFlags.playback)
                this.logger.playbackLogNote('shrd_notInTxGroup', `${txQueueEntry.logID}`, ``);
              ...
              if (
                ...
                if (txQueueEntry.ourNodeInTransactionGroup === false) {
                  nestedCountersInstance.countEvent(
                    'transactionQueue',
                    'spread_tx_to_group_syncing-skipped2'
                  );
                  ...
                  if (idxModEveryN > 0) {
                    nestedCountersInstance.countEvent(
                      'transactionQueue',
                      'spread_tx_to_group_syncing-skipped'
                    );
                    send_spread_tx_to_group_syncing = false;
                  }
                }
                if (send_spread_tx_to_group_syncing) {
                  nestedCountersInstance.countEvent(
                    'transactionQueue',
                    'spread_tx_to_group_syncing-notSkipped'
                  );
                  ...
                  if (txQueueEntry.globalModification === false) {
                    if (logFlags.verbose)
                      this.mainLogger.debug(
                        `routeAndQueueAcceptedTransaction: spread_tx_to_group ${txQueueEntry.logID}`
                      );
                    ...
                    if (logFlags.playback)
                      this.logger.playbackLogNote(
                        'shrd_sync_tx',
                        `${txQueueEntry.logID}`,
                        `txts: ${timestamp} nodes:${utils.stringifyReduce(cycleShardData.syncingNeighborsTxGroup.map((x) => x.id))}`
                      );
                    ...
                    if (
                      ...
                      if (logFlags.seqdiagram) {
                        for (const node of cycleShardData.syncingNeighborsTxGroup) {
                          if (logFlags.seqdiagram)
                            this.seqLogger.info(
                              `0x53455102 ${shardusGetTime()} tx:${acceptedTx.txId} ${NodeList.activeIdToPartition.get(Self.id)}-->>${NodeList.activeIdToPartition.get(node.id)}: ${'spread_tx_to_group_syncing'}`
                            );
                        }
                      }
                    ...
                    if (logFlags.verbose)
                      this.mainLogger.debug(
                        `routeAndQueueAcceptedTransaction: bugfix detected. avoid forwarding txs where globalModification == true ${txQueueEntry.logID}`
                      );
                  }
                }
              }
        ...
        if (logFlags.seqdiagram)
          this.seqLogger.info(
            `0x53455105 ${shardusGetTime()} tx:${txQueueEntry.acceptedTx.txId} Note over ${NodeList.activeIdToPartition.get(Self.id)}: pendingQ`
          );
        ...
        if (logFlags.playback)
          this.logger.playbackLogNote(
            'shrd_txPreQueued',
            `${txQueueEntry.logID}`,
            `${txQueueEntry.logID} gm:${txQueueEntry.globalModification}`
          );
        ...
        if (logFlags.playback)
          this.logger.playbackLogNote(
            'shrd_addtoqueue_rejected',
            `${txId}`,
            `AcceptedTransaction: ${txQueueEntry.logID} ts: ${txQueueEntry.txKeys.timestamp} acc: ${utils.stringifyReduce(txQueueEntry.txKeys.allKeys)}`
          );
        this.statemanager_fatal(
      ...
      for (const key of txQueueEntry.shardusMemoryPatternSets.ri) {
        ...
        if (logFlags.verbose)
          this.mainLogger.info(
            `queueEntryPrePush: fetching immutable data for tx ${txQueueEntry.acceptedTx.txId} key ${key}`
          );
  ...
  getQueueEntrySafe(txid: string): QueueEntry | null {
    ...
    if (queueEntry === undefined) {
      ...
      if (queueEntry === undefined) {
        if (logFlags.debug)
          this.mainLogger.debug(`getQueueEntrySafe failed to find: ${utils.stringifyReduce(txid)}`);
  ...
  getQueueEntryArchived(txid: string, msg: string): QueueEntry | null {
    ...
    if (logFlags.error)
      this.mainLogger.error(
        `getQueueEntryArchived failed to find: ${utils.stringifyReduce(txid)} ${msg} dbg:${this.stateManager.debugTXHistory[utils.stringifyReduce(txid)]}`
      );
    return null;
  }
      ...
      for (const queueEntry of this.archivedQueueEntriesByID.values()) {
        if (queueEntry.uniqueKeys.includes(accountId)) {
          ...
          if (receipt2 != null) {
            ...
            if (receipt2.appliedVote)
              nestedCountersInstance.countEvent(
                'getArchivedQueueEntryByAccountIdAndHash',
                'get vote from' + ' receipt2'
              );
          }
  ...
  queueEntryAddData(queueEntry: QueueEntry, data: Shardus.WrappedResponse, signatureCheck = false): void {
    ...
    if (logFlags.playback)
      this.logger.playbackLogNote(
        'shrd_addData',
        `${queueEntry.logID}`,
        `key ${utils.makeShortHash(data.accountId)} hash: ${utils.makeShortHash(data.stateId)} hasAll:${queueEntry.hasAll} collected:${queueEntry.dataCollected}  ${queueEntry.acceptedTx.timestamp}`
      );
    profilerInstance.profileSectionStart('queueEntryAddData', true);
  }
  ...
  async queueEntryRequestMissingData(queueEntry: QueueEntry): Promise<void> {
    ...
    if (logFlags.playback)
      this.logger.playbackLogNote(
        'shrd_queueEntryRequestMissingData_start',
        `${queueEntry.acceptedTx.txId}`,
        `qId: ${queueEntry.entryID} AccountsMissing:${utils.stringifyReduce(allKeys)}`
      );
    ...
    for (const key of queueEntry.uniqueKeys) {
      ...
      if (queueEntry.collectedData[key] == null && queueEntry.requests[key] == null) {
        ...
        while (keepTrying) {
          ...
          if (logFlags.playback)
            this.logger.playbackLogNote(
              'shrd_queueEntryRequestMissingData_ask',
              `${queueEntry.logID}`,
          ...
          try {
            if (this.config.p2p.useBinarySerializedEndpoints && this.config.p2p.requestStateForTxBinary) {
              ...
              if (logFlags.seqdiagram)
                this.seqLogger.info(
                  `0x53455101 ${shardusGetTime()} tx:${message.txid} ${NodeList.activeIdToPartition.get(Self.id)}-->>${NodeList.activeIdToPartition.get(node.id)}: ${'request_state_for_tx'}`
                );
            ...
            if (logFlags.error) this.mainLogger.error('askBinary request_state_for_tx exception:', error);

            if (logFlags.error)
              this.mainLogger.error(
                `askBinary error: ${InternalRouteEnum.binary_request_state_for_tx} asked to ${node.externalIp}:${node.externalPort}:${node.id}`
              );
          }
          ...
          if (result == null) {
            ...
            if (logFlags.playback)
              this.logger.playbackLogNote(
                'shrd_queueEntryRequestMissingData_askfailretry',
                `${queueEntry.logID}`,
                `r:${relationString}   asking: ${utils.makeShortHash(node.id)} qId: ${queueEntry.entryID} `
              );
            continue;
          }
          if (result.success !== true) {
            ...
            if (logFlags.playback)
              this.logger.playbackLogNote(
                'shrd_queueEntryRequestMissingData_askfailretry2',
                `${queueEntry.logID}`,
                `r:${relationString}   asking: ${utils.makeShortHash(node.id)} qId: ${queueEntry.entryID} `
              );
            continue;
          ...
          if (logFlags.playback)
            this.logger.playbackLogNote(
              'shrd_queueEntryRequestMissingData_result',
              `${queueEntry.logID}`,
              `r:${relationString}   result:${queueEntry.logstate} dataCount:${dataCountReturned} asking: ${utils.makeShortHash(node.id)} qId: ${queueEntry.entryID}  AccountsMissing:${utils.stringifyReduce(allKeys)} AccountsReturned:${utils.stringifyReduce(accountIdsReturned)}`
            );
  ...
  async queueEntryRequestMissingReceipt(queueEntry: QueueEntry): Promise<void> {
    ...
    if (logFlags.playback)
      this.logger.playbackLogNote(
        'shrd_queueEntryRequestMissingReceipt_start',
        `${queueEntry.acceptedTx.txId}`,
        `qId: ${queueEntry.entryID}`
      );
    ...
    for (const key of queueEntry.uniqueKeys) {
      ...
      while (keepTrying) {
        ...
        if (logFlags.playback)
          this.logger.playbackLogNote(
            'shrd_queueEntryRequestMissingReceipt_ask',
            `${queueEntry.logID}`,
            `r:${relationString}   asking: ${utils.makeShortHash(node.id)} qId: ${queueEntry.entryID} `
          );
        ...
        if (
          ...
          try {
            if (logFlags.seqdiagram)
              this.seqLogger.info(
                `0x53455101 ${shardusGetTime()} tx:${message.txid} ${NodeList.activeIdToPartition.get(Self.id)}-->>${NodeList.activeIdToPartition.get(node.id)}: ${'request_receipt_for_tx'}`
              );
            ...
            this.mainLogger.error(
              `askBinary error: ${InternalRouteEnum.binary_request_receipt_for_tx} asked to ${node.externalIp}:${node.externalPort}:${node.id}`
            );
          }
        } else {
        ...
        if (result == null) {
          if (logFlags.verbose) {
            if (logFlags.error)
              this.mainLogger.error(
                `ASK FAIL request_receipt_for_tx ${triesLeft} ${utils.makeShortHash(node.id)}`
              );
          }
          if (logFlags.playback)
            this.logger.playbackLogNote(
              'shrd_queueEntryRequestMissingReceipt_askfailretry',
              `${queueEntry.logID}`,
              `r:${relationString}   asking: ${utils.makeShortHash(node.id)} qId: ${queueEntry.entryID} `
            );
          continue;
        ...
        if (result.success !== true) {
          if (logFlags.error)
            this.mainLogger.error(
              `ASK FAIL queueEntryRequestMissingReceipt 9 ${triesLeft} ${utils.makeShortHash(node.id)}:${utils.makeShortHash(node.internalPort)} note:${result.note} txid:${queueEntry.logID}`
            );
          continue;
        }
        ...
        if (logFlags.playback)
          this.logger.playbackLogNote(
            'shrd_queueEntryRequestMissingReceipt_result',
            `${queueEntry.logID}`,
            `r:${relationString}   result:${queueEntry.logstate} asking: ${utils.makeShortHash(node.id)} qId: ${queueEntry.entryID} result: ${utils.stringifyReduce(result)}`
          );
      ...
      while (keepTrying) {
        ...
        if (result == null) {
          if (logFlags.verbose) {
            if (logFlags.error)
              this.mainLogger.error(
                `ASK FAIL request_receipt_for_tx_old ${triesLeft} ${utils.makeShortHash(node.id)}`
              );
          }
          if (logFlags.playback)
            this.logger.playbackLogNote(
  ...
  queueEntryGetTransactionGroup(queueEntry: QueueEntry, tryUpdate = false): Shardus.Node[] {
    ...
    for (const key of queueEntry.uniqueKeys) {
      ...
      if (queueEntry.globalModification === false) {
        if (this.stateManager.accountGlobals.isGlobalAccount(key) === true) {
          if (logFlags.verbose)
            this.mainLogger.debug(
              `queueEntryGetTransactionGroup skipping: ${utils.makeShortHash(key)} tx: ${queueEntry.logID}`
            );
          continue;
        } else {
      ...
      for (const node of homeNode.nodeThatStoreOurParitionFull) {
        ...
        if (node.id === Self.id)
          if (logFlags.verbose)
            this.mainLogger.debug(
              `queueEntryGetTransactionGroup tx ${queueEntry.logID} our node coverage key ${key}`
            );
      }
      ...
      if (
        ...
        if (logFlags.verbose)
          this.mainLogger.debug(
            `queueEntryGetTransactionGroup executeInOneShard ${queueEntry.logID} isInExecutionHome:${queueEntry.isInExecutionHome} executionGroup:${Utils.safeStringify(executionKeys)}`
          );
        if (logFlags.playback && logFlags.verbose)
          this.logger.playbackLogNote(
            'queueEntryGetTransactionGroup',
    ...
    if (uniqueNodes[cycleShardData.ourNode.id] == null) {
      ...
      if (logFlags.verbose)
        this.mainLogger.debug(
          `queueEntryGetTransactionGroup not involved: hasNonG:${hasNonGlobalKeys} tx ${queueEntry.logID}`
        );
    }
    if (queueEntry.ourNodeInTransactionGroup)
      if (logFlags.seqdiagram)
        this.seqLogger.info(
          `0x53455105 ${shardusGetTime()} tx:${queueEntry.acceptedTx.txId} Note over ${NodeList.activeIdToPartition.get(Self.id)}: targetgroup`
        );
  ...
  queueEntryGetConsensusGroup(queueEntry: QueueEntry): Shardus.Node[] {
    ...
    for (const key of queueEntry.uniqueKeys) {
      ...
      if (queueEntry.globalModification === false) {
        if (this.stateManager.accountGlobals.isGlobalAccount(key) === true) {
          if (logFlags.verbose)
            this.mainLogger.debug(
              `queueEntryGetConsensusGroup skipping: ${utils.makeShortHash(key)} tx: ${queueEntry.logID}`
            );
          continue;
        } else {
          hasNonGlobalKeys = true;
    ...
    if (uniqueNodes[cycleShardData.ourNode.id] == null) {
      ...
      if (logFlags.verbose)
        this.mainLogger.debug(
          `queueEntryGetConsensusGroup not involved: hasNonG:${hasNonGlobalKeys} tx ${queueEntry.logID}`
        );
    }
  ...
  queueEntryGetConsensusGroupForAccount(queueEntry: QueueEntry, accountId: string): Shardus.Node[] {
    ...
    if (queueEntry.uniqueKeys.includes(accountId) === false) {
      throw new Error(
        `queueEntryGetConsensusGroup: account ${accountId} is not in the queueEntry.uniqueKeys`
      );
    }
    const txGroup = [];
    const uniqueNodes: StringNodeObjectMap = {};
  ...
  //        this.setDebugLastAwaitedCallInner('this.stateManager.transactionQueue.app.getRelevantData old')
  //       let data = await this.app.getRelevantData(
  //         key,
  //         queueEntry.acceptedTx.data,
  //         queueEntry.acceptedTx.appData
  //       )
  //        this.setDebugLastAwaitedCallInner('this.stateManager.transactionQueue.app.getRelevantData old', DebugComplete.Completed)
      ...
      if (logFlags.seqdiagram) {
        for (const node of nodes) {
          if (context == 'tellCorrespondingNodes') {
            if (logFlags.seqdiagram)
              this.seqLogger.info(
                `0x53455102 ${shardusGetTime()} tx:${message.txid} ${NodeList.activeIdToPartition.get(Self.id)}-->>${NodeList.activeIdToPartition.get(node.id)}: ${'broadcast_state_nodes'}`
              );
          } else {
            if (logFlags.seqdiagram)
              this.seqLogger.info(
                `0x53455102 ${shardusGetTime()} tx:${message.txid} ${NodeList.activeIdToPartition.get(Self.id)}-->>${NodeList.activeIdToPartition.get(node.id)}: ${'broadcast_state_neighbour'}`
              );
          }
        }
      }
  ...
  async tellCorrespondingNodes(queueEntry: QueueEntry): Promise<unknown> {
    ...
    for (const key of queueEntry.uniqueKeys) {
      ...
      if (this.stateManager.accountGlobals.isGlobalAccount(key)) {
        ...
        isGlobalKey = true;
        if (logFlags.playback)
          this.logger.playbackLogNote('globalAccountMap', queueEntry.logID, `tellCorrespondingNodes - has`);
      }
      ...
      if (hasKey === false) {
        if (loggedPartition === false) {
          ...
          if (logFlags.verbose)
            this.mainLogger.debug(
              `tellCorrespondingNodes hasKey=false: ${utils.stringifyReduce(homeNode.nodeThatStoreOurParitionFull.map((v) => v.id))}`
            );
          if (logFlags.verbose)
            this.mainLogger.debug(
              `tellCorrespondingNodes hasKey=false: full: ${utils.stringifyReduce(homeNode.nodeThatStoreOurParitionFull)}`
            );
        }
        if (logFlags.verbose)
          this.mainLogger.debug(`tellCorrespondingNodes hasKey=false  key: ${utils.stringifyReduce(key)}`);
      }
      ...
      if (hasKey) {
        ...
        this.setDebugLastAwaitedCallInner(
          'this.stateManager.transactionQueue.app.getRelevantData',
          DebugComplete.Completed
        );
        this.profiler.scopedProfileSectionEnd('process_dapp.getRelevantData');
        this.profiler.profileSectionEnd('process_dapp.getRelevantData');
    ...
    if (queueEntry.globalModification === true) {
      if (logFlags.playback)
        this.logger.playbackLogNote(
          'tellCorrespondingNodes',
          queueEntry.logID,
          `tellCorrespondingNodes - globalModification = true, not telling other nodes`
        );
      return;
    }
          ...
          if (key !== key2) {
            ...
            if (correspondingAccNodes.length > 0) {
              ...
              if (filteredNodes.length === 0) {
                if (logFlags.error)
                  this.mainLogger.error(
                    'tellCorrespondingNodes: filterValidNodesForInternalMessage no valid nodes left to try'
                  );
                return null;
              }
              const filterdCorrespondingAccNodes = filteredNodes;
  ...
  async factTellCorrespondingNodes(queueEntry: QueueEntry): Promise<unknown> {
    try {
      ...
      const dataValuesWeHave = [];
      const datas: { [accountID: string]: Shardus.WrappedResponse } = {};
      const remoteShardsByKey: { [accountID: string]: StateManagerTypes.shardFunctionTypes.NodeShardData } =
        {}; // shard homenodes that we do not have the data for.
      let loggedPartition = false;
      for (const key of queueEntry.uniqueKeys) {
        let hasKey = ShardFunctions.testAddressInRange(key, ourNodeData.storedPartitions);
        ...
        if (this.stateManager.accountGlobals.isGlobalAccount(key)) {
          ...
          if (logFlags.playback)
            this.logger.playbackLogNote(
              'globalAccountMap',
              queueEntry.logID,
              `factTellCorrespondingNodes - has`
            );
        }
        ...
        if (hasKey === false) {
          if (loggedPartition === false) {
            loggedPartition = true;
            if (logFlags.verbose) this.mainLogger.debug(`factTellCorrespondingNodes hasKey=false`);
          }
          if (logFlags.verbose)
            this.mainLogger.debug(
              `factTellCorrespondingNodes hasKey=false  key: ${utils.stringifyReduce(key)}`
            );
        }
      ...
      if (queueEntry.globalModification === true) {
        if (logFlags.playback)
          this.logger.playbackLogNote(
            ...
            queueEntry.logID,
            `factTellCorrespondingNodes - globalModification = true, not telling other nodes`
          );
        return;
      }
      ...
      if (logFlags.verbose) {
        this.mainLogger.debug(
          `factTellCorrespondingNodes: target group size`,
          targetGroup.length,
          targetGroup
        );
        this.mainLogger.debug(
      ...
      for (const index of validCorrespondingIndices) {
        ...
        if (this.config.stateManager.filterReceivingNodesForTXData) {
          ...
          for (const wrappedResponse of signedPayload.stateList) {
            ...
            const targetHasKey = ShardFunctions.testAddressInRange(
              accountId,
              targetNodeShardData.storedPartitions
            );
      ...
      this.mainLogger.debug(
        `factTellCorrespondingNodes: correspondingIndices and nodes ${queueEntry.logID}`,
        ourIndexInTxGroup,
        correspondingIndices,
        correspondingNodes.map((n) => n.id),
        callParams
      );
      ...
      if (filteredNodes.length === 0) {
        if (logFlags.error)
          this.mainLogger.error(
            'factTellCorrespondingNodes: filterValidNodesForInternalMessage no valid nodes left to try'
          );
        nestedCountersInstance.countEvent(
          'stateManager',
          'factTellCorrespondingNodes: no corresponding nodes needed to send'
      ...
      if (payload.stateList.length === 0) {
        if (logFlags.error)
          this.mainLogger.error(
            'factTellCorrespondingNodes: filterValidNodesForInternalMessage payload.stateList.length === 0'
          );
        nestedCountersInstance.countEvent(
          'stateManager',
          'factTellCorrespondingNodes: payload.stateList.length === 0'
        );
        return null;
      }
      ...
      this.statemanager_fatal(
        `factTellCorrespondingNodes_ex`,
        'factTellCorrespondingNodes' + utils.formatErrorMessage(error)
      );
    }
  }
  ...
  validateCorrespondingTellSender(queueEntry: QueueEntry, dataKey: string, senderNodeId: string): boolean {
    if (logFlags.verbose)
      this.mainLogger.debug(
        `validateCorrespondingTellSender: data key: ${dataKey} sender node id: ${senderNodeId}`
      );
    ...
    if (configContext.stateManager.shareCompleteData) {
      ...
      if (senderIsInExecutionGroup && neighbourNodeIds.includes(senderNodeId) === false) {
        this.mainLogger.error(
          `validateCorrespondingTellSender: sender is an execution node but not a neighbour node`
        );
        return false;
      }
      ...
      if (logFlags.verbose)
        this.mainLogger.debug(
          `validateCorrespondingTellSender: data key: ${dataKey} sender node id: ${senderNodeId} senderHasAddress: ${senderHasAddress} receiverIsInExecutionGroup: ${receiverIsInExecutionGroup} senderIsInExecutionGroup: ${senderIsInExecutionGroup}`
        );
      if (
        receiverIsInExecutionGroup === true ||
        senderHasAddress === true ||
        senderIsInExecutionGroup === true
      ) {
        return true;
      }
    } else {
      if (logFlags.verbose)
        this.mainLogger.debug(
          `validateCorrespondingTellSender: data key: ${dataKey} sender node id: ${senderNodeId} senderHasAddress: ${senderHasAddress} receiverIsInExecutionGroup: ${receiverIsInExecutionGroup}`
        );
  ...
  factValidateCorrespondingTellSender(
    ...
    if (logFlags.verbose)
      this.mainLogger.debug(
        `factValidateCorrespondingTellSender: txId: ${queueEntry.acceptedTx.txId} sender node id: ${senderNodeId}, receiver id: ${Self.id}`
      );
    let cycleShardData = this.stateManager.currentCycleShardData;
    if (Context.config.stateManager.deterministicTXCycleEnabled) {
      cycleShardData = this.stateManager.shardValuesByCycle.get(queueEntry.txGroupCycle);
    ...
    this.mainLogger.debug(
      `factValidateCorrespondingTellSender: txId: ${queueEntry.acceptedTx.txId} sender node id: ${senderNodeId}, receiver id: ${Self.id} senderHasAddress: ${senderHasAddress} receivingNodeIndex: ${receivingNodeIndex} senderNodeIndex: ${senderNodeIndex} receiverGroupSize: ${receiverGroupSize} senderGroupSize: ${senderGroupSize} targetIndices: ${utils.stringifyReduce(targetIndices)}`
    );
  ...
  getStartAndEndIndexOfTargetGroup(
    ...
    if (logFlags.verbose)
      this.mainLogger.debug(`getStartAndEndIndexOfTargetGroup: all target indexes`, targetIndexes);
  ...
  async tellCorrespondingNodesFinalData(queueEntry: QueueEntry): Promise<void> {
    ...
    if (logFlags.playback)
      this.logger.playbackLogNote(
        ...
        queueEntry.logID,
        `tellCorrespondingNodesFinalData - start: ${queueEntry.logID}`
      );
      ...
      if (wrappedStates[key] != null) {
        ...
        if (correspondingAccNodes.length > 0) {
          ...
          if (filteredNodes.length === 0) {
            if (logFlags.error)
              this.mainLogger.error(
                'tellCorrespondingNodesFinalData: filterValidNodesForInternalMessage no valid nodes left to try'
              );
            //return null
            continue;
          }
          ...
          if (logFlags.error)
            this.mainLogger.debug(
              'tellcorrernodingnodesfinaldata',
              queueEntry.logID,
              ` : filterValidNodesForInternalMessage ${filterNodesIpPort} for accounts: ${utils.stringifyReduce(message.stateList)}`
            );
          if (this.config.p2p.useBinarySerializedEndpoints && this.config.p2p.broadcastFinalStateBinary) {
            ...
            if (logFlags.seqdiagram) {
              for (const node of filterdCorrespondingAccNodes) {
                if (logFlags.seqdiagram)
                  this.seqLogger.info(
                    `0x53455102 ${shardusGetTime()} tx:${message.txid} ${NodeList.activeIdToPartition.get(Self.id)}-->>${NodeList.activeIdToPartition.get(node.id)}: ${'broadcast_finalstate'}`
                  );
              }
            }
    ...
    if (logFlags.verbose)
      this.mainLogger.debug(
        `tellCorrespondingNodesFinalData - end: ${queueEntry.logID} totalShares:${totalShares}`
      );
  ...
  factTellCorrespondingNodesFinalData(queueEntry: QueueEntry): void {
      ...
      if (wrappedStates[key] != null) {
        ...
        if (logFlags.verbose)
          if (logFlags.playback) {
            this.logger.playbackLogNote(
              'factTellCorrespondingNodesFinalData',
              queueEntry.logID,
              `factTellCorrespondingNodesFinalData ourIndex: ${senderIndexInTxGroup} correspondingIndices:${JSON.stringify(correspondingIndices)} correspondingNodes:${JSON.stringify(correspondingNodes.map((node) => node.id))} for accounts: ${key}`
            );
          }
        ...
        if (correspondingNodes.length > 0) {
          ...
          if (filteredNodes.length === 0) {
            if (logFlags.error)
              this.mainLogger.error(
                'factTellCorrespondingNodesFinalData: filterValidNodesForInternalMessage no valid nodes left to try'
              );
            //return null
            continue;
          }
          ...
          if (
            ...
            Context.config.p2p.poqoDataAndReceiptBinary
          ) {
            ...
            this.p2p.tell(filterdCorrespondingAccNodes, 'poqo-data-and-receipt', {
              finalState: message,
              receipt: queueEntry.appliedReceipt2,
            });
          } else if (
            this.config.p2p.useBinarySerializedEndpoints &&
            this.config.p2p.broadcastFinalStateBinary
          ) {
            this.p2p.tellBinary<BroadcastFinalStateReq>(
              filterdCorrespondingAccNodes,
              InternalRouteEnum.binary_broadcast_finalstate,
    ...
    if (logFlags.verbose)
      this.mainLogger.debug(
        `factTellCorrespondingNodesFinalData - end: ${queueEntry.logID} totalShares:${totalShares}`
      );
  ...
  factValidateCorrespondingTellFinalDataSender(queueEntry: QueueEntry, senderNodeId: string): boolean {
    if (logFlags.verbose)
      this.mainLogger.debug(
        `factValidateCorrespondingTellFinalDataSender: txId: ${queueEntry.acceptedTx.txId} sender node id: ${senderNodeId}, receiver id: ${Self.id}`
      );
    const senderNode = NodeList.nodes.get(senderNodeId);
    if (senderNode === null) {
      if (logFlags.error)
        this.mainLogger.error(
          `factValidateCorrespondingTellFinalDataSender: logId: ${queueEntry.logID} sender node is null`
        );
      nestedCountersInstance.countEvent(
        'stateManager',
    ...
    if (senderIsInExecutionGroup === false) {
      if (logFlags.error)
        this.mainLogger.error(
          `factValidateCorrespondingTellFinalDataSender: logId: ${queueEntry.logID} sender is not in the execution group sender:${senderNodeId}`
        );
      nestedCountersInstance.countEvent(
        'stateManager',
    ...
    if (isValidFactSender === false) {
      if (logFlags.error)
        this.mainLogger.error(
          `factValidateCorrespondingTellFinalDataSender: logId: ${queueEntry.logID} sender is not a valid sender isValidSender:  ${isValidFactSender}`
        );
      nestedCountersInstance.countEvent(
        'stateManager',
  ...
  removeFromQueue(queueEntry: QueueEntry, currentIndex: number, archive = true): void {
    ...
    if (logFlags.seqdiagram)
      this.seqLogger.info(
        `0x53455104 ${shardusGetTime()} tx:${queueEntry.acceptedTx.txId} Note over ${NodeList.activeIdToPartition.get(Self.id)}: removed`
      );
    ...
    if (logFlags.debug)
      this.mainLogger.debug(`removeFromQueue: ${queueEntry.logID} and added to archive done`);
  }
  ...
  async processTransactions(firstTime = false): Promise<void> {
    ...
    try {
      ...
      if (this.pendingTransactionQueue.length > 5000) {
        nestedCountersInstance.countEvent(
          'stateManager',
          `newAcceptedTxQueueTempInjest>5000 leftRunning:${this.transactionProcessingQueueRunning} noShardCalcs:${this.stateManager.currentCycleShardData == null} `
        );
        ...
        for (const txQueueEntry of this.pendingTransactionQueue) {
          ...
          if (age > timeM * 0.9) {
            ...
            if (txQueueEntry.didSync == false) {
              ...
              if (logFlags.playback)
                this.logger.playbackLogNote(
                  'shrd_processAcceptedTxQueueTooOld1',
                  `${utils.makeShortHash(txQueueEntry.acceptedTx.txId)}`,
                  'processAcceptedTxQueue working on older tx ' + timestamp + ' age: ' + age
                );
          ...
          if (age > timeM) {
            if (logFlags.playback)
              this.logger.playbackLogNote(
                'shrd_processAcceptedTxQueueTooOld2',
                `${utils.makeShortHash(txQueueEntry.acceptedTx.txId)}`,
                'processAcceptedTxQueue working on older tx ' + timestamp + ' age: ' + age
              );
          ...
          if (logFlags.seqdiagram)
            this.seqLogger.info(
              `0x53455105 ${shardusGetTime()} tx:${txQueueEntry.acceptedTx.txId} Note over ${NodeList.activeIdToPartition.get(Self.id)}: aging`
            );
          ...
          if (logFlags.playback)
            this.logger.playbackLogNote(
              'shrd_addToQueue',
              `${txId}`,
              `AcceptedTransaction: ${txQueueEntry.logID} ts: ${txQueueEntry.txKeys.timestamp} acc: ${utils.stringifyReduce(txQueueEntry.txKeys.allKeys)} indexInserted: ${index + 1}`
            );
      ...
      while (this._transactionQueue.length > 0) {
        ...
        if (logFlags.seqdiagram)
          this.mainLogger.info(
            `0x10052024 ${ipInfo.externalIp} ${shardusGetTime()} 0x0001 currentIndex:${currentIndex} txId:${queueEntry.acceptedTx.txId} state:${queueEntry.state}`
          );
        ...
        if (localRestartCounter < this.queueRestartCounter && lastLog !== this.queueRestartCounter) {
          ...
          if (logFlags.playback)
            this.logger.playbackLogNote(
              'queueRestart_error',
              `${queueEntry.acceptedTx.txId}`,
              `qId: ${queueEntry.entryID} qRst:${localRestartCounter}  qrstGlobal:${this.queueRestartCounter}}`
            );
        }
            ...
            if (txAge > timeM3 * 2 && queueEntry.didSync == false) {
              ...
              if (logFlags.playback)
                this.logger.playbackLogNote(
                  'txExpired',
                  `${shortID}`,
                  `${queueEntry.txGroupDebug} txExpired ${utils.stringifyReduce(queueEntry.acceptedTx)}`
                );
              if (logFlags.playback)
                this.logger.playbackLogNote(
                  'txExpired',
                  `${shortID}`,
                  `${queueEntry.txGroupDebug} queueEntry.recievedAppliedReceipt: ${utils.stringifyReduce(queueEntry.recievedAppliedReceipt)}`
                );

              nestedCountersInstance.countEvent(
                'txExpired',
                `> M3 * 2. NormalTX Timed out. didSync == false. state:${queueEntry.state} globalMod:${queueEntry.globalModification}`
              );
              ...
              if (txAge > timeM3 && queueEntry.requestingReceiptFailed) {
                ...
                if (logFlags.playback)
                  this.logger.playbackLogNote(
                    'txExpired',
                    `${shortID}`,
                    `${queueEntry.txGroupDebug} txExpired 3 requestingReceiptFailed  ${utils.stringifyReduce(queueEntry.acceptedTx)} ${queueEntry.didWakeup}`
                  );
                if (logFlags.playback)
                  this.logger.playbackLogNote(
                    'txExpired',
                    `${shortID}`,
                    `${queueEntry.txGroupDebug} queueEntry.recievedAppliedReceipt 3 requestingReceiptFailed: ${utils.stringifyReduce(queueEntry.recievedAppliedReceipt)}`
                  );

                nestedCountersInstance.countEvent(
                  'txExpired',
                  `> M3. receiptRequestFail after Timed Out. state:${queueEntry.state} globalMod:${queueEntry.globalModification}`
                );
              ...
              if (txAge > timeM3 && queueEntry.repairFailed) {
                ...
                if (logFlags.playback)
                  this.logger.playbackLogNote(
                    'txExpired',
                    `${shortID}`,
                    `${queueEntry.txGroupDebug} txExpired 3 repairFailed  ${utils.stringifyReduce(queueEntry.acceptedTx)} ${queueEntry.didWakeup}`
                  );
                if (logFlags.playback)
                  this.logger.playbackLogNote(
                    'txExpired',
                    `${shortID}`,
                    `${queueEntry.txGroupDebug} queueEntry.recievedAppliedReceipt 3 repairFailed: ${utils.stringifyReduce(queueEntry.recievedAppliedReceipt)}`
                  );

                nestedCountersInstance.countEvent(
                  'txExpired',
                  `> M3. repairFailed after Timed Out. state:${queueEntry.state} globalMod:${queueEntry.globalModification}`
                );
                    ...
                    if (queueEntry.recievedAppliedReceipt == null && queueEntry.appliedReceipt == null) {
                      ...
                      if (logFlags.playback)
                        this.logger.playbackLogNote(
                          'txMissingReceipt3',
                          `${shortID}`,
                          `processAcceptedTxQueue ` +
                            `txid: ${shortID} state: ${queueEntry.state} applyReceipt:${hasApplyReceipt} recievedAppliedReceipt:${hasReceivedApplyReceipt} age:${txAge}`
                        );

                      nestedCountersInstance.countEvent(
                        'txMissingReceipt',
                        `Wait for reciept only: txAge > timeM2.5. state:${queueEntry.state} globalMod:${queueEntry.globalModification}`
                      );
                      queueEntry.waitForReceiptOnly = true;
                ...
                if (
                  ...
                  if (
                    ...
                    if (logFlags.verbose)
                      if (logFlags.error)
                        this.mainLogger.error(
                          `txAge > timeM3 => ask for receipt now ` +
                            `txid: ${shortID} state: ${queueEntry.state} applyReceipt:${hasApplyReceipt} recievedAppliedReceipt:${hasReceivedApplyReceipt} age:${txAge}`
                        );
                    if (logFlags.playback)
                      this.logger.playbackLogNote(
                        'txMissingReceipt1',
                        `txAge > timeM3 ${shortID}`,
                        `syncNeedsReceipt ${shortID}`
                      );
                    ...
                    nestedCountersInstance.countEvent(
                      'txMissingReceipt',
                      `txAge > timeM3 => ask for receipt now. state:${queueEntry.state} globalMod:${queueEntry.globalModification} seen:${seen}`
                    );
                    queueEntry.waitForReceiptOnly = true;
                    queueEntry.m2TimeoutReached = true;
            ...
            if (txAge > timeM3 * 50) {
              ...
              if (logFlags.playback)
                this.logger.playbackLogNote(
                  'txExpired',
                  `${shortID}`,
                  `${queueEntry.txGroupDebug} txExpired 4  ${utils.stringifyReduce(queueEntry.acceptedTx)}`
                );
              if (logFlags.playback)
                this.logger.playbackLogNote(
                  'txExpired',
                  `${shortID}`,
                  `${queueEntry.txGroupDebug} queueEntry.recievedAppliedReceipt 4: ${utils.stringifyReduce(queueEntry.recievedAppliedReceipt)}`
                );

              nestedCountersInstance.countEvent(
                'txExpired',
                `txExpired txAge > timeM3 * 50. still syncing. state:${queueEntry.state} globalMod:${queueEntry.globalModification}`
              );
        ...
        if (this.queueTimingFixes === true) {
            ...
            if (this.processQueue_accountSeen(seenAccounts, queueEntry) === true) {
              ...
              if (txAge > timeM2 + queueEntry.txSieveTime) {
                if (configContext.stateManager.disableTxExpiration === false) {
                  nestedCountersInstance.countEvent(
                    'txExpired',
                    `> M2 canceled due to upstream TXs. state:${queueEntry.state} hasAll:${queueEntry.hasAll} globalMod:${queueEntry.globalModification}`
                  );
                  //todo only keep on for temporarliy
                  nestedCountersInstance.countEvent(
                    'txExpired',
                    `> M2 canceled due to upstream TXs. sieveT:${queueEntry.txSieveTime}`
                  );
          ...
          if (
            ...
            this.statemanager_fatal(
              `txSafelyRemoved`,
              `txAge > 2m txid: ${shortID} state: ${queueEntry.state} age:${txAge}`
            );
            this.removeFromQueue(queueEntry, currentIndex);
            continue;
          }
          ...
          if (configContext.stateManager.removeStuckTxsFromQueue2 === true) {
            const timeSinceLastVoteMessage =
              queueEntry.lastVoteReceivedTimestamp > 0
                ? currentTime - queueEntry.lastVoteReceivedTimestamp
                : 0;
            ...
            if (timeSinceLastVoteMessage > configContext.stateManager.stuckTxRemoveTime2) {
              nestedCountersInstance.countEvent(
                'txSafelyRemoved',
                `tx waiting for votes more than ${configContext.stateManager.stuckTxRemoveTime2 / 1000} seconds. state: ${queueEntry.state}`
              );
              this.statemanager_fatal(
                `txSafelyRemoved`,
                `stuck in consensus. 2. waiting for votes. txid: ${shortID} state: ${queueEntry.state} age:${txAge} tx first vote seen ${timeSinceLastVoteMessage / 1000} seconds ago`
              );
              this.removeFromQueue(queueEntry, currentIndex);
              continue;
            }
          ...
          if (configContext.stateManager.removeStuckTxsFromQueue3 === true) {
            if (queueEntry.state === 'consensing' && txAge > configContext.stateManager.stuckTxRemoveTime3) {
              ...
              nestedCountersInstance.countEvent(
                'txSafelyRemoved',
                `tx in consensus more than ${configContext.stateManager.stuckTxRemoveTime3 / 1000} seconds. state: ${queueEntry.state} has seen vote: ${anyVotes}`
              );
              this.statemanager_fatal(
                `txSafelyRemoved`,
                `stuck in consensus. 3. txid: ${shortID} state: ${queueEntry.state} age:${txAge}`
              );
              this.removeFromQueue(queueEntry, currentIndex);
              continue;
            }
                ...
                if (configContext.stateManager.stuckTxQueueFix) {
                  if (configContext.stateManager.singleAccountStuckFix) {
                    ...
                    if (
                      ...
                      nestedCountersInstance.countEvent(
                        'consensus',
                        `move stuck consensing tx to await final data.`
                      );
            ...
            if (expireTx) {
              ...
              if (logFlags.playback)
                this.logger.playbackLogNote(
                  'txExpired',
                  `${shortID}`,
                  `${queueEntry.txGroupDebug} txExpired >m2 fail ${reason}  ${utils.stringifyReduce(queueEntry.acceptedTx)} ${queueEntry.didWakeup}`
                );
              ...
              nestedCountersInstance.countEvent(
                'txExpired',
                `> timeM2 fail ${reason} state:${queueEntry.state} hasAll:${queueEntry.hasAll} globalMod:${queueEntry.globalModification} `
              );
            ...
            if (!receivedVote && !hasVoted && queueEntry.almostExpired == null) {
              ...
              if (logFlags.playback)
                this.logger.playbackLogNote(
                  'txExpired',
                  `${shortID}`,
                  `setTxAlmostExpired ${queueEntry.txGroupDebug} txExpired 3 requestingReceiptFailed  ${utils.stringifyReduce(queueEntry.acceptedTx)} ${queueEntry.didWakeup}`
                );
              //if (logFlags.playback) this.logger.playbackLogNote('txExpired', `${shortID}`, `${queueEntry.txGroupDebug} queueEntry.recievedAppliedReceipt 3 requestingReceiptFailed: ${utils.stringifyReduce(queueEntry.recievedAppliedReceipt)}`)

              nestedCountersInstance.countEvent(
                'txExpired',
                `setTxAlmostExpired > M3. general case state:${queueEntry.state} hasAll:${queueEntry.hasAll} globalMod:${queueEntry.globalModification} hasReceipt:${hasReceipt} matchingReceipt:${matchingReceipt} isInExecutionHome:${isInExecutionHome} hasVote: ${queueEntry.receivedBestVote != null}`
              );
              nestedCountersInstance.countEvent(
                'txExpired',
                `setTxAlmostExpired > M3. general case sieveT:${queueEntry.txSieveTime} extraTime:${extraTime}`
              );
            ...
            if (this.processQueue_accountSeen(seenAccounts, queueEntry) === false) {
              ...
              try {
                ...
                if (this.executeInOneShard === true) {
                  this.setDebugLastAwaitedCall(
                    'this.stateManager.transactionQueue.tellCorrespondingNodes(queueEntry)'
                  );
                  ...
                  this.setDebugLastAwaitedCall(
                    'this.stateManager.transactionQueue.tellCorrespondingNodesOld(queueEntry)'
                  );
                ...
                if (logFlags.debug)
                  this.mainLogger.debug(
                    `tellCorrespondingNodes: ${queueEntry.logID} dataSharedTimestamp: ${queueEntry.dataSharedTimestamp}`
                  );
                ...
                if (logFlags.verbose)
                  if (logFlags.playback)
                    this.logger.playbackLogNote(
                      'shrd_processing',
                      `${shortID}`,
                      `qId: ${queueEntry.entryID} qRst:${localRestartCounter}  values: ${this.processQueue_debugAccountData(queueEntry, app)}`
                    );
                //}
              } catch (ex) {
                if (logFlags.debug)
                ...
                if (
                  ...
                  if (logFlags.debug)
                    this.mainLogger.debug(
                      `processAcceptedTxQueue2 isInExecutionHome === false. set state = 'consensing' tx:${queueEntry.logID} ts:${queueEntry.acceptedTx.timestamp}`
                    );
              ...
              if (upstreamTx == null) {
                if (logFlags.seqdiagram && queueEntry?.upStreamBlocker !== 'null') {
                  ...
                  this.seqLogger.info(
                    `0x53455104 ${shardusGetTime()} tx:${queueEntry.acceptedTx.txId} Note over ${NodeList.activeIdToPartition.get(Self.id)}: upstream:null`
                  );
                }
                nestedCountersInstance.countEvent(
                  'processing',
                  'busy waiting the upstream tx.' + ' but it is null'
                );
              } else {
                if (upstreamTx.logID === queueEntry.logID) {
                  if (logFlags.seqdiagram && queueEntry?.upStreamBlocker !== upstreamTx.logID) {
                    ...
                    this.seqLogger.info(
                      `0x53455104 ${shardusGetTime()} tx:${queueEntry.acceptedTx.txId} Note over ${NodeList.activeIdToPartition.get(Self.id)}: upstream:same`
                    );
                  }
                  ...
                  if (upstreamTx === queueEntry) {
                    ...
                    nestedCountersInstance.countEvent(
                      'processing',
                      'busy waiting the upstream tx but it is same txId'
                    );
                  }
                } else {
                  if (logFlags.seqdiagram && queueEntry?.upStreamBlocker !== upstreamTx.logID) {
                    queueEntry.upStreamBlocker = upstreamTx.logID;
                    this.seqLogger.info(
                      `0x53455104 ${shardusGetTime()} tx:${queueEntry.acceptedTx.txId} Note over ${NodeList.activeIdToPartition.get(Self.id)}: upstream:${upstreamTx.logID}`
                    );
                  }
                  nestedCountersInstance.countEvent(
                    'processing',
          ...
          if (queueEntry.state === 'awaiting data') {
            ...
            if (queueEntry.hasAll === false && txAge > timeM2) {
              ...
              if (this.queueEntryHasAllData(queueEntry) === true) {
                ...
                nestedCountersInstance.countEvent(
                  'processing',
                  'data missing at t>M2. but not really. investigate further'
                );
                if (logFlags.playback)
                  this.logger.playbackLogNote(
                    'shrd_hadDataAfterall',
                    `${shortID}`,
                    `This is kind of an error, and should not happen`
                  );
                continue;
              }
              ...
              if (this.config.stateManager.awaitingDataCanBailOnReceipt) {
                ...
                if (receipt != null) {
                  ...
                  this.updateTxState(
                    queueEntry,
                    'await final data',
                    'receipt while waiting for initial data'
                  );
                  continue;
                }
              }
              ...
              if (this.processQueue_accountSeen(seenAccounts, queueEntry) === false) {
                ...
                if (logFlags.verbose)
                  if (logFlags.playback)
                    this.logger.playbackLogNote(
                      'shrd_preApplyTx',
                      `${shortID}`,
                      `qId: ${queueEntry.entryID} qRst:${localRestartCounter} values: ${this.processQueue_debugAccountData(queueEntry, app)} AcceptedTransaction: ${utils.stringifyReduce(queueEntry.acceptedTx)}`
                    );
                ...
                try {
                  ...
                  const awaitStart = shardusGetTime();
                  this.setDebugLastAwaitedCall(
                    'this.stateManager.transactionQueue.preApplyTransaction(queueEntry)'
                  );
                  ...
                  if (
                    ...
                    for (const key of Object.keys(queueEntry.collectedData)) {
                      ...
                      const { timestamp, hash } = this.app.getTimestampAndHashFromAccount(
                        wrappedAccount.data
                      );
                    ...
                    if (queueEntry.noConsensus === true) {
                      ...
                      if (logFlags.verbose)
                        if (logFlags.playback)
                          this.logger.playbackLogNote('shrd_preApplyTx_noConsensus', `${shortID}`, ``);
                      ...
                      if (logFlags.verbose)
                        if (logFlags.playback)
                          this.logger.playbackLogNote('shrd_preApplyTx_createAndShareVote', `${shortID}`, ``);
                      if (logFlags.debug)
                        this.mainLogger.debug(
                          `processAcceptedTxQueue2 calling createAndShareVote : ${queueEntry.logID} `
                        );
                      const awaitStart = shardusGetTime();
                    ...
                    nestedCountersInstance.countEvent(
                      'processing',
                      `txResult apply error. applied: ${txResult?.applied}`
                    );
                    if (logFlags.error)
                      this.mainLogger.error(
                        `processAcceptedTxQueue2 txResult problem txid:${queueEntry.logID} res: ${utils.stringifyReduce(txResult)} `
                      );
                    queueEntry.waitForReceiptOnly = true;
            ...
            if (this.processQueue_accountSeen(seenAccounts, queueEntry) === false) {
              ...
              if (this.usePOQo) {
                ...
                if (
                  queueEntry.pendingConfirmOrChallenge.size > 0 &&
                  queueEntry.robustQueryVoteCompleted === true &&
                  queueEntry.acceptVoteMessage === false
                ) {
                  this.mainLogger.debug(
                    `processAcceptedTxQueue2 consensing : ${queueEntry.logID} pendingConfirmOrChallenge.size = ${queueEntry.pendingConfirmOrChallenge.size}`
                  );
                  for (const [nodeId, confirmOrChallenge] of queueEntry.pendingConfirmOrChallenge) {
                    const appendSuccessful = this.stateManager.transactionConsensus.tryAppendMessage(
                      queueEntry,
                      confirmOrChallenge
                    );
                    if (appendSuccessful) {
                  ...
                  this.mainLogger.debug(
                    `processAcceptedTxQueue2 consensing : ${queueEntry.logID} reset pendingConfirmOrChallenge.size = ${queueEntry.pendingConfirmOrChallenge.size}`
                  );
                }
              ...
              if (logFlags.debug)
                this.mainLogger.debug(
                  `processAcceptedTxQueue2 tryProduceReceipt result : ${queueEntry.logID} ${utils.stringifyReduce(result)}`
                );
              ...
              if (this.stateManager.getReceipt2(queueEntry) != null) {
                ...
                if (queueEntry.isInExecutionHome) {
                  if (this.usePOQo) {
                    ...
                    if (numberOfSharingNodes > queueEntry.executionGroup.length)
                      numberOfSharingNodes = queueEntry.executionGroup.length;
                    const highestRankedNodeIds = queueEntry.executionGroup
                      .slice(0, numberOfSharingNodes)
                      .map((n) => n.id);
                ...
                if (isChallengedReceipt && this.useNewPOQ) {
                  ...
                  nestedCountersInstance.countEvent(
                    'consensus',
                    'isChallengedReceipt: true removing from queue'
                  );
                  this.mainLogger.debug(
                    `processAcceptedTxQueue2 tryProduceReceipt isChallengedReceipt : ${logID}. remove from queue`
                  );
                  continue;
                }
                ...
                if (isReceiptMatchPreApply && queueEntry.isInExecutionHome) {
                  ...
                  if (logFlags.verbose)
                    if (logFlags.playback)
                      this.logger.playbackLogNote(
                        'shrd_consensingComplete_madeReceipt',
                        `${shortID}`,
                        `qId: ${queueEntry.entryID}  `
                      );
                  ...
                  if (
                    ...
                    if (logFlags.verbose)
                      if (logFlags.playback)
                        this.logger.playbackLogNote(
                          'shrd_consensingComplete_finishedFailReceipt1',
                          `${shortID}`,
                          `qId: ${queueEntry.entryID}  `
                        );
                    ...
                    if (logFlags.debug || this.stateManager.consensusLog) {
                      this.mainLogger.debug(
                        `processAcceptedTxQueue2 tryProduceReceipt failed result: false : ${queueEntry.logID} ${utils.stringifyReduce(result)}`
                      );
                      this.statemanager_fatal(
                        `processAcceptedTxQueue2`,
                        `tryProduceReceipt failed result: false : ${queueEntry.logID} ${utils.stringifyReduce(result)}`
                      );
                    }
                    nestedCountersInstance.countEvent(
                      'consensus',
                  ...
                  if (logFlags.verbose)
                    if (logFlags.playback)
                      this.logger.playbackLogNote(
                        'shrd_consensingComplete_gotReceiptNoMatch1',
                        `${shortID}`,
                        `qId: ${queueEntry.entryID}  `
                      );
                  if (this.stateManager.getReceiptResult(queueEntry) === false) {
                    ...
                    if (logFlags.verbose)
                      if (logFlags.playback)
                        this.logger.playbackLogNote(
                          'shrd_consensingComplete_finishedFailReceipt2',
                          `${shortID}`,
                          `qId: ${queueEntry.entryID}  `
                        );
                    // we are finished since there is nothing to apply
              ...
              if (finishedConsensing === false) {
                ...
                if (hasReceivedApplyReceipt && queueEntry.recievedAppliedReceipt != null) {
                  if (
                    ...
                    if (logFlags.verbose)
                      if (logFlags.playback)
                        this.logger.playbackLogNote(
                          'shrd_consensingComplete_gotReceipt',
                          `${shortID}`,
                          `qId: ${queueEntry.entryID} `
                        );
                    ...
                    if (logFlags.verbose)
                      if (logFlags.playback)
                        this.logger.playbackLogNote(
                          'shrd_consensingComplete_gotReceiptNoMatch2',
                          `${shortID}`,
                          `qId: ${queueEntry.entryID}  `
                        );
                    didNotMatchReceipt = true;
                ...
                if (didNotMatchReceipt === true && queueEntry.isInExecutionHome) {
                  ...
                  if (logFlags.verbose)
                    if (logFlags.playback)
                      this.logger.playbackLogNote(
                        'shrd_consensingComplete_didNotMatchReceipt',
                        `${shortID}`,
                        `qId: ${queueEntry.entryID} result:${queueEntry.appliedReceiptForRepair.result} `
                      );
                  ...
                  if (queueEntry.appliedReceiptForRepair.result === true) {
                    ...
                    if (logFlags.verbose)
                      if (logFlags.playback)
                        this.logger.playbackLogNote(
                          'shrd_consensingComplete_finishedFailReceipt3',
                          `${shortID}`,
                          `qId: ${queueEntry.entryID}  `
                        );
                    // we are finished since there is nothing to apply
          ...
          if (queueEntry.state === 'await repair') {
            ...
            if (queueEntry.repairFinished === true) {
              if (logFlags.verbose)
                if (logFlags.playback)
                  this.logger.playbackLogNote(
                    'shrd_awaitRepair_repairFinished',
                    `${shortID}`,
                    `qId: ${queueEntry.entryID} result:${queueEntry.appliedReceiptForRepair.result} txAge:${txAge} `
                  );
              if (queueEntry.appliedReceiptForRepair.result === true) {
          ...
          if (queueEntry.state === 'await final data') {
            ...
            if (this.processQueue_accountSeen(seenAccounts, queueEntry) === false) {
              ...
              if (vote) {
                ...
                if (incomplete && missingAccounts.length > 0) {
                  ...
                  if (timeSinceAwaitFinalStart > 5000) {
                    ...
                    if (logFlags.verbose)
                      this.mainLogger.debug(
                        `shrd_awaitFinalData_incomplete : ${queueEntry.logID} starting finalDataRequest timeSinceDataShare: ${timeSinceAwaitFinalStart}`
                      );
                    ...
                    if (logFlags.verbose)
                      this.mainLogger.debug(
                        `shrd_awaitFinalData_incomplete : ${queueEntry.logID} starting finalDataRequest txAge > timeM3 + confirmationSeenExpirationTime`
                      );
                  }

                  // start request process for missing data
                  const timeSinceLastFinalDataRequest =
                    shardusGetTime() - queueEntry.lastFinalDataRequestTimestamp;
                  if (
                  ...
                  nestedCountersInstance.countEvent(
                    'stateManager',
                    'shrd_awaitFinalData not missing accounts'
                  );
                }
                ...
                if (failed === true) {
                  ...
                  if (logFlags.verbose)
                    if (logFlags.playback)
                      this.logger.playbackLogNote(
                        'shrd_awaitFinalData_failed',
                        `${shortID}`,
                        `qId: ${queueEntry.entryID} skipped:${skipped}`
                      );
                  if (logFlags.debug)
                    this.mainLogger.debug(`shrd_awaitFinalData_failed : ${queueEntry.logID} `);
                  continue;
                }
                ...
                if (failed === false && incomplete === false) {
                  ...
                  if (logFlags.verbose)
                    if (logFlags.playback)
                      this.logger.playbackLogNote(
                        'shrd_awaitFinalData_passed',
                        `${shortID}`,
                        `qId: ${queueEntry.entryID} skipped:${skipped}`
                      );
                  if (logFlags.debug)
                    this.mainLogger.debug(
                      `shrd_awaitFinalData_passed : ${queueEntry.logID} skipped:${skipped}`
                    );
                  ...
                  if (logFlags.verbose)
                    console.log('transactionReceiptPass 1', queueEntry.acceptedTx.txId, queueEntry);
                  this.updateSimpleStatsObject(
                    processStats.awaitStats,
                  ...
                  if (
                    ...
                    if (logFlags.debug)
                      this.mainLogger.error(
                        `shrd_awaitFinalData_fail : ${queueEntry.logID} no receivedAppliedRecipt or recievedAppliedReceipt2. appliedReceipt2: ${utils.stringifyReduce(queueEntry.appliedReceipt2)}`
                      );
                    this.updateTxState(queueEntry, 'fail');
                  }
                  this.removeFromQueue(queueEntry, currentIndex);
              ...
              if (upstreamTx == null) {
                ...
                nestedCountersInstance.countEvent(
                  'stateManager',
                  'shrd_awaitFinalData busy. upstreamTx: null'
                );
              } else {
                if (upstreamTx.acceptedTx.txId === queueEntry.acceptedTx.txId) {
                  nestedCountersInstance.countEvent(
                    'stateManager',
                    'shrd_awaitFinalData busy. upstreamTx same tx'
                  );
                } else {
                  nestedCountersInstance.countEvent(
                    'stateManager',
            ...
            if (this.processQueue_accountSeen(seenAccounts, queueEntry) === false) {
              ...
              try {
                ...
                if (queueEntry.noConsensus === true) {
                  ...
                  if (logFlags.debug)
                    this.mainLogger.debug(
                      `processAcceptedTxQueue2 commiting finished : noConsensus:${queueEntry.state} ${queueEntry.logID} `
                    );
                } else if (queueEntry.appliedReceipt2 != null) {
                  // the final state of the queue entry will be pass or fail based on the receipt
                  ...
                  if (logFlags.debug)
                    this.mainLogger.debug(
                      `processAcceptedTxQueue2 commiting finished : Recpt:${queueEntry.state} ${queueEntry.logID} `
                    );
                } else if (queueEntry.recievedAppliedReceipt2 != null) {
                  // the final state of the queue entry will be pass or fail based on the receipt
                  ...
                  if (logFlags.debug)
                    this.mainLogger.debug(
                      `processAcceptedTxQueue2 commiting finished : recvRecpt:${queueEntry.state} ${queueEntry.logID} `
                    );
                } else {
                  this.updateTxState(queueEntry, 'fail');
                  if (logFlags.error)
                    this.mainLogger.error(
                      `processAcceptedTxQueue2 commiting finished : no receipt ${queueEntry.logID} `
                    );
                }
      ...
      if (processTime > 10000) {
        ...
        if (logFlags.error)
          this.mainLogger.error(
            `processTime > 5s ${processTime / 1000} stats:${Utils.safeStringify(processStats)}`
          );
        ...
        if (logFlags.error && logFlags.verbose)
          this.mainLogger.error(
            `processTime > 2s ${processTime / 1000} stats:${Utils.safeStringify(processStats)}`
          );
        ...
        if (logFlags.error && logFlags.verbose)
          this.mainLogger.error(
            `processTime > 1s ${processTime / 1000} stats:${Utils.safeStringify(processStats)}`
          );
  ...
  private setTXExpired(queueEntry: QueueEntry, currentIndex: number, message: string): void {
    if (logFlags.verbose || this.stateManager.consensusLog)
      this.mainLogger.debug(
        `setTXExpired tx:${queueEntry.logID} ${message}  ts:${queueEntry.acceptedTx.timestamp} debug:${utils.stringifyReduce(queueEntry.debug)} state: ${queueEntry.state}, isInExecution: ${queueEntry.isInExecutionHome}`
      );
    this.updateTxState(queueEntry, 'expired');
    this.removeFromQueue(queueEntry, currentIndex);
    ...
    nestedCountersInstance.countEvent(
      'txExpired',
      `tx: ${this.app.getSimpleTxDebugValue(queueEntry.acceptedTx?.data)}`
    );
    ...
    if (queueEntry.appliedReceiptFinal2 != null) {
      ...
      if (logFlags.debug)
        this.mainLogger.debug(`setTXExpired. ${queueEntry.logID} start repair:${startRepair}. update `);
      if (startRepair) {
      ...
      if (logFlags.debug) this.mainLogger.debug(`setTXExpired. no receipt to repair ${queueEntry.logID}`);
    }
  }
  private setTxAlmostExpired(queueEntry: QueueEntry, currentIndex: number, message: string): void {
    if (logFlags.verbose || this.stateManager.consensusLog)
      this.mainLogger.debug(
        `setTxAlmostExpired tx:${queueEntry.logID} ${message}  ts:${queueEntry.acceptedTx.timestamp} debug:${utils.stringifyReduce(queueEntry.debug)}`
      );
  ...
  getArchiverReceiptFromQueueEntry(queueEntry: QueueEntry): ArchiverReceipt | null {
    ...
    const receipt2 = this.stateManager.getReceipt2(queueEntry);
    const appliedReceipt = receipt2
      ? Utils.safeJsonParse(Utils.safeStringify(receipt2))
      : ({} as AppliedReceipt2);
  ...
  async requestFinalData(
    ...
    try {
      ...
      if (logFlags.debug)
        this.mainLogger.debug(
          `requestFinalData: txid: ${queueEntry.acceptedTx.txId} accountIds: ${utils.stringifyReduce(accountIds)}, asking node: ${nodeToAsk.id} ${nodeToAsk.externalPort} at timestamp ${shardusGetTime()}`
        );
      ...
      if (this.config.p2p.useBinarySerializedEndpoints && this.config.p2p.requestTxAndStateBinary) {
        ...
        if (logFlags.seqdiagram)
          this.seqLogger.info(
            `0x53455101 ${shardusGetTime()} tx:${queueEntry.acceptedTx.txId} ${NodeList.activeIdToPartition.get(Self.id)}-->>${NodeList.activeIdToPartition.get(nodeToAsk.id)}: ${'request_tx_and_state'}`
          );
      ...
      if (response && response.stateList && response.stateList.length > 0) {
        this.mainLogger.debug(
          `requestFinalData: txid: ${queueEntry.logID} received data for ${response.stateList.length} accounts`
        );
      } else {
      ...
      for (const data of response.stateList) {
        ...
        if (queueEntry.collectedFinalData[data.accountId] == null) {
          ...
          if (logFlags.debug)
            this.mainLogger.debug(
              `requestFinalData: txid: ${queueEntry.logID} success accountId: ${data.accountId} stateId: ${data.stateId}`
            );
        }
      }
      if (successCount === accountIds.length) {
      ...
      if (success === false) {
        ...
        this.mainLogger.error(
          `requestFinalData: txid: ${queueEntry.logID} failed. successCount: ${successCount} accountIds: ${accountIds.length}`
        );
      }
    }
  ...
  processQueue_clearAccountsSeen(seenAccounts: SeenAccounts, queueEntry: QueueEntry): void {
    ...
    for (const key of queueEntry.uniqueKeys) {
      if (seenAccounts[key] != null && seenAccounts[key].logID === queueEntry.logID) {
        if (logFlags.verbose)
          this.mainLogger.debug(`${new Date()}}clearing key ${key} for tx ${queueEntry.logID}`);
  ...
  getAccountQueueCount(accountID: string, remote = false): QueueCountsResult {
    ...
    for (const queueEntry of this.pendingTransactionQueue) {
      if (queueEntry.txKeys.sourceKeys.length > 0 && accountID === queueEntry.txKeys.sourceKeys[0]) {
        ...
        if (logFlags.verbose)
          console.log(
            'getAccountQueueCount: found upstream tx in the injested queue:',
            `appData: ${Utils.safeStringify(tx.appData)}`
          );
        count++;
      }
    }
    for (const queueEntry of this._transactionQueue) {
      if (queueEntry.txKeys.sourceKeys.length > 0 && accountID === queueEntry.txKeys.sourceKeys[0]) {
        ...
        if (logFlags.verbose)
          console.log(
            'getAccountQueueCount: found upstream tx in the newAccepted queue:',
            `appData: ${Utils.safeStringify(tx.appData)}`
          );
        count++;
      }
    ...
    if (logFlags.verbose)
      console.log(`getAccountQueueCount: remote:${remote} ${count} acc:${utils.stringifyReduce(accountID)}`);
  ...
  isAccountInQueue(accountID: string, remote = false): boolean {
    for (const queueEntry of this.pendingTransactionQueue) {
      if (queueEntry.uniqueKeys.includes(accountID)) {
        ...
        if (queueEntry.txKeys.sourceKeys.length > 0 && accountID === queueEntry.txKeys.sourceKeys[0]) {
          if (logFlags.verbose)
            console.log('isAccountInQueue: found upstream tx in the' + ' injested' + ' queue:');
    ...
    for (const queueEntry of this._transactionQueue) {
      if (queueEntry.uniqueKeys.includes(accountID)) {
        ...
        if (queueEntry.txKeys.sourceKeys.length > 0 && accountID === queueEntry.txKeys.sourceKeys[0]) {
          if (logFlags.verbose)
            console.log('isAccountInQueue: found upstream tx in the' + ' newAccepted' + ' queue:');
  ...
  getDebugQueueInfo(queueEntry: QueueEntry): any {
    return {
      ...
      simpleDebugStr: this.app.getSimpleTxDebugValue
        ? this.app.getSimpleTxDebugValue(queueEntry.acceptedTx?.data)
        : '',
    };
  }
  // eslint-disable-next-line @typescript-eslint/explicit-function-return-type
  ...
  updateTxState(queueEntry: QueueEntry, nextState: string, context = ''): void {
    if (logFlags.seqdiagram)
      if (context == '')
        if (logFlags.seqdiagram)
          this.seqLogger.info(
            `0x53455104 ${shardusGetTime()} tx:${queueEntry.acceptedTx.txId} Note over ${NodeList.activeIdToPartition.get(Self.id)}: ${queueEntry.state}-${nextState}`
          );
        else if (logFlags.seqdiagram)
          this.seqLogger.info(
            `0x53455104 ${shardusGetTime()} tx:${queueEntry.acceptedTx.txId} Note over ${NodeList.activeIdToPartition.get(Self.id)}: ${queueEntry.state}-${nextState}:${context}`
          );
