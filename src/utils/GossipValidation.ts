...
export function validateGossipPayload<T extends GossipPayload>(payload: T | JoinRequest, validationSchema: Record<string, string>, logContext: string): boolean {
  if (!payload) {
  ...
  if (![1, 2].includes(CycleCreator.currentQuarter)) {
    ...
    if (isJoinReq) {
      ...
      if (config.debug.cycleRecordOOSDebugLogs) {
        console.log(`DEBUG CR-OOS: ${logContext}-reject: not in Q1 or Q2 currentQuarter: ${CycleCreator.currentQuarter} payload publicKey: ${payload.nodeInfo.publicKey || undefined}`);
      }
    }
    // Handle the case where the payload is not a JoinRequest
    else if (!isJoinReq && config.debug.cycleRecordOOSDebugLogs) {
      console.log(`DEBUG CR-OOS: ${logContext}-reject: not in Q1 or Q2 currentQuarter: ${CycleCreator.currentQuarter} payload id: ${(payload as GossipPayload).nodeId || undefined} payload cycle: ${(payload as GossipPayload).cycleNumber || undefined}`);
    }
    // Log warnings if error logging is enabled
    if (logFlags.error) {
      if (isJoinRequest(payload)) {
        warn(`${logContext}-reject: not in Q1 or Q2 currentQuarter: ${CycleCreator.currentQuarter} payload publicKey: ${payload.nodeInfo.publicKey || undefined}`);
      } else {
        warn(`${logContext}-reject: not in Q1 or Q2 currentQuarter: ${CycleCreator.currentQuarter} payload id: ${(payload as GossipPayload).nodeId || undefined} payload cycle: ${(payload as GossipPayload).cycleNumber || undefined}`);
      }
    }
    return false;
  }
...
export function verifyOriginalSenderAndQuarter(payload: GossipPayload | JoinRequest, sender: string, context: string): boolean {
