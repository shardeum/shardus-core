...
//let queuedProposals: SignedApoptosisProposal[] =  []
...
// [TODO] - validate input
  ...
  if (!(payload as LooseObject).when) return 
  const when = payload.when
  if (when > currentCycle+1 || when < currentCycle-1) return 
...
export function init() {
  ...
  for (const route of routes.internal) {
    network.registerInternal(route.name, route.handler)
  }
...
export async function apoptosizeSelf() {
  const activeNodes = activeByIdOrder  // [TODO] - maybe we should shuffle this array
