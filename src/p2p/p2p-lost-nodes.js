...
class P2PLostNodes {
  ...
  _validateLostMessage(lostMsg, verify = true) {
    ...
    if (!source) return [false, '"source" value is falsy']
    if (!target) return [false, '"target" value is falsy']
    if (!investigator) return [false, '"investigator" value is falsy']
    if (!lastCycleMarker) return [false, '"lastCycleMarker" value is falsy']
    if (!cycleCounter) return [false, '"cycleCounter" value is falsy']
    ...
    if (!myLastCycle) return [false, '"cycleCounter" is not present in cycles']
    if (lastCycleMarker !== myLastCycle.marker) return [false, '"lastCycleMarker" does not match marker in cycles']
    // Ensure investigator is computed correctly
    const myInvestigator = this._findLostNodeInvestigator(target, myLastCycle.marker)
    if (myInvestigator !== investigator) return [false, '"investigator" is not correct for the given "target" and "cycleMarker"']
    // Verify the signature, if requested
    if (verify) {
      if (!this.crypto.verify(lostMsg)) return [false, 'Invalid signature on lostMsg']
    }
    return [true, 'All good']
  }
  ...
  validateDownMessage(downMsg) {
    ...
    if (!this.crypto.verify(downMsg)) return [false, 'Invalid signature on downMsg']
  ...
  validateUpMessage(upMsg) {
    ...
    const lostMsg = downMsg.lostMessage
    if (!lostMsg) return [false, '"lostMessage" field is missing']
    if (!this.crypto.verify(upMsg)) return [false, 'Invalid signature on upMsg']
