...
function validateScalingRequest(scalingRequest: P2P.CycleAutoScaleTypes.SignedScaleRequest): boolean {
  ...
  if (
    ...
    warn(
      `Invalid scaling request, not a valid scaling type. Request: ${Utils.safeStringify(scalingRequest)}`
    );
    return false;
  }
...
function _addToScalingRequests(scalingRequest): boolean {
  switch (scalingRequest.scale) {
    ...
    default:
      warn(
        `Invalid scaling type in _addToScalingRequests(). Request: ${Utils.safeStringify(scalingRequest)}`
      );
      return false;
  }
}
