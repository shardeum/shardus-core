...
const validatorListHashRoute: P2P.P2PTypes.Route<Handler> = {
  ...
  handler: (_req, res) => {
    ...
    res.send(Utils.safeStringify({ nodeListHash: NodeList.getNodeListHash(), nextCycleTimestamp }))
  },
}
...
const archiverListHashRoute: P2P.P2PTypes.Route<Handler> = {
  ...
  handler: (_req, res) => {
    res.send(Utils.safeStringify({ archiverListHash: Archivers.getArchiverListHash() }))
  },
}
...
const standbyListHashRoute: P2P.P2PTypes.Route<Handler> = {
  ...
  handler: (_req, res) => {
    res.send(Utils.safeStringify({ standbyNodeListHash: JoinV2.getStandbyListHash() }))
  },
}
...
const newestCycleHashRoute: P2P.P2PTypes.Route<Handler> = {
  ...
  handler: (_req, res) => {
    res.send(Utils.safeStringify(CycleChain.getCurrentCycleMarker()))
  },
}
...
const archiverListRoute: P2P.P2PTypes.Route<Handler> = {
  ...
  handler: (req, res) => {
    ...
    if (expectedHash && expectedHash === Archivers.getArchiverListHash()) {
      res.send(Utils.safeStringify(Archivers.getLastHashedArchiverList()))
    } else {
...
const cycleByMarkerRoute: P2P.P2PTypes.Route<Handler> = {
  ...
  handler: (req, res) => {
    ...
    if (cycle) {
      res.send(Utils.safeStringify(cycle))
    } else {
...
const newestCycleRecordRoute: P2P.P2PTypes.Route<Handler> = {
  ...
  handler: (_req, res) => {
    ...
    res.send(Utils.safeStringify(CycleChain.newest))
