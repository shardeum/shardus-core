...
const lostArchiverUpGossip: GossipHandler<SignedObject<ArchiverUpMsg>, Node['id']> = (
  ...
  // check args
  if (!payload) {
    logging.warn(`lostArchiverUpGossip: missing payload`);
    return;
  }
  if (!sender) {
...
const lostArchiverDownGossip: GossipHandler<SignedObject<ArchiverDownMsg>, Node['id']> = (
  ...
  logging.info(
    `lostArchiverDownGossip: payload: ${inspect(payload)}, sender: ${sender}, tracker: ${tracker}`
  );

  // check args
  if (!payload) {
    logging.warn(`lostArchiverDownGossip: missing payload`);
    return;
  }
  if (!sender) {
...
const investigateLostArchiverRoute: Route<InternalHandler<SignedObject<InvestigateArchiverMsg>>> = {
  ...
  handler: (payload, response, sender) => {
    ...
    // check args
    if (!payload) {
      logging.warn(`investigateLostArchiverRoute: missing payload`);
      return;
    }
    if (!response) {
      logging.warn(`investigateLostArchiverRoute: missing response`);
      return;
    }
    if (!sender) {
      logging.warn(`investigateLostArchiverRoute: missing sender`);
      return;
    }
    ...
    if (error) {
      nestedCountersInstance.countEvent(
        'lostArchivers',
        `investigateLostArchiverRoute invalid payload ${error}`
      );
      logging.warn(
