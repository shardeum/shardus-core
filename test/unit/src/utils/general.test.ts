...
test('compareObjectShape() > should detect missing property', () => {
  ...
  expect(StringUtils.safeStringify(expectedError) === StringUtils.safeStringify(error)).toBe(true)
})
...
test('generateObjectSchema() > should generate proper object schema', () => {
  ...
  const isEqual = StringUtils.safeStringify(generatedSchema) === StringUtils.safeStringify(expectedSchema)
